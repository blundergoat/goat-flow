 ...
 * `isTargetUntrusted` to the "static" vs default (runtime) deny-mechanism evidence
...
describe("audit --untrusted-target flag", () => {
  it("defaults off so an ordinary audit keeps its runtime deny proof", () => {
    assert.equal(parseCLIArgs(["audit", "."]).isTargetUntrusted, false);
  });

  it("turns on to skip executing an untrusted checkout's hook code", () => {
    assert.equal(
      parseCLIArgs(["audit", ".", "--untrusted-target"]).isTargetUntrusted,
      true,
    );
  });
});
