...
describe("loadSnapshotFacts (real repo)", () => {
  ...
  it("documents the current release snapshot as a frozen copy", () => {
    const snapshot = loadSnapshotJson("1.8.0");
    assert.equal(typeof snapshot._snapshot_note, "string");
    assert.match(
      snapshot._snapshot_note,
      /Frozen copy of workflow\/manifest\.json as it shipped at v1\.8\.0 release/,
    );
    assert.ok(
