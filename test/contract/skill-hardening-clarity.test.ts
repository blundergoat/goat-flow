...
describe("skill hardening contracts: goat-clarity", () => {
  ...
  it("reports test-value dispositions without mutating test meaning", () => {
    assertIncludesAll([
      ...
      "report-only",
      "`test-selection.md`'s four-part value gate and replacement safeguards",
    ...
    const selectionPath = "workflow/skills/playbooks/test-selection.md";
    assertGuidanceIncludesAll(readProjectFile(selectionPath), selectionPath, [
      ...
      "keep the original until replacement coverage proves the same invariant and passes",
      "added-test, or removed-test disposition",
    ]);
  ...
  it("returns a complete but proportional remediation receipt", () => {
    ...
    assertIncludesAll([
      ...
      "Complete the reference's formatter-capability record",
      "`READY` requires baseline and final results",
      "`NOT_FOUND` requires discovery evidence",
