...
function splitLegacyFootgunDir(content: string): { preamble: string; entries: string[] } {
  const firstIndex = content.search(/^##\s*Footgun:/m);
  const preamble = firstIndex >= 0 ? content.slice(0, firstIndex).trim() : '# Footguns';
  ...
  const raw = content.slice(firstIndex).split(/^##\s*Footgun:\s*/m).filter(Boolean);
  return {
    preamble: preamble || '# Footguns',
    entries: raw,
  };
...
function splitLegacyLessonDir(content: string): { preamble: string; entries: { heading: string; body: string; isPattern: boolean }[] } {
  const entriesIndex = content.search(/^##\s*Entries\b/im);
  const preamble = entriesIndex >= 0 ? content.slice(0, entriesIndex).trim() : '# Lessons';
  ...
  if (headings.length === 0) {
    return {
      ...
      entries: [{ heading: 'Legacy lesson fixture', body: source, isPattern: false }],
    };
  }
...
function maybeSplitLegacyLearningLoopFolders(files: Record<string, string>): Record<string, string> {
  const split = { ...files };
  const hasNestedPath = (dir: string): boolean => {
    return Object.keys(split).some(key => key.startsWith(dir) && key.length > dir.length);
  };
  ...
  if (split[LEGACY_FOOTGUNS_DIR] && !hasNestedPath(LEGACY_FOOTGUNS_DIR)) {
    ...
    if (raw && looksLikeLegacyFootgunDir) {
      ...
      for (const rawEntry of entries) {
        ...
        const nameRaw = firstLineEnd >= 0 ? rawEntry.slice(0, firstLineEnd).trim() : 'Legacy footgun fixture';
        const name = nameRaw || 'Legacy footgun fixture';
        let body = firstLineEnd >= 0 ? rawEntry.slice(firstLineEnd + 1) : rawEntry;
        const statusMatch = body.match(/^\*\*Status:\*\*\s*(.+)\n?/im);
        const createdMatch = body.match(/^\*\*Created:\*\*\s*(.+)\n?/im);
        const evidenceMatch = body.match(/^\*\*Evidence type:\*\*\s*(.+)\n?/im);
        const status = statusMatch?.[1]?.trim().toLowerCase().startsWith('resolved') ? 'resolved' : 'active';
        ...
        split[`${LEGACY_FOOTGUNS_DIR}${file}`] = `${frontmatter}${body ? `${body}\n` : ''}`.trimEnd() + '\n';
      }
  ...
  if (split[LEGACY_LESSONS_DIR] && !hasNestedPath(LEGACY_LESSONS_DIR)) {
    ...
    if (raw) {
      ...
      for (const entry of parsed.entries) {
        const heading = entry.heading.replace(/^Pattern:\s*/i, '').trim() || 'Legacy lesson fixture';
        let body = entry.body;
        const createdMatch = body.match(/^\*\*created_at:\*\*\s*(.+)\n?/im) ?? body.match(/^created_at:\s*(.+)\n?/im);
        const created = createdMatch?.[1]?.trim() ?? '';
        ...
        split[`${LEGACY_LESSONS_DIR}${filename}`] = `${frontmatter}${body ? `${body}\n` : ''}`.trimEnd() + '\n';
      }
...
function detectAgents(files: Record<string, string>): string[] {
  const agents: string[] = [];
  if ('CLAUDE.md' in files || Object.keys(files).some(k => k.startsWith('.claude/'))) agents.push('claude');
  if ('AGENTS.md' in files || Object.keys(files).some(k => k.startsWith('.agents/'))) agents.push('codex');
  if ('GEMINI.md' in files || Object.keys(files).some(k => k.startsWith('.gemini/'))) agents.push('gemini');
  return agents;
}

function normalizeLearningLoop(files: Record<string, string>): Record<string, string> {
  const normalized = maybeSplitLegacyLearningLoopFolders({ ...files });

  if (normalized[LEGACY_FOOTGUNS_FILE] && !Object.keys(normalized).some(k => k.startsWith('docs/footguns/'))) {
    normalized['docs/footguns/README.md'] = '# Footguns\n\nLegacy test fixture imported into directory layout.\n';
  ...
  if (normalized[LEGACY_LESSONS_FILE] && !Object.keys(normalized).some(k => k.startsWith('ai/lessons/'))) {
    normalized['ai/lessons/README.md'] = '# Lessons\n\nLegacy test fixture imported into directory layout.\n';
  ...
  if (hasLearningLoop && !('.goat-flow/config.yaml' in normalized)) {
    ...
    normalized['.goat-flow/config.yaml'] = [
      ...
      ...(agents.length > 0 ? ['agents:', ...agents.map(agent => `  - ${agent}`)] : []),
      'skills:',
      '  install: all',
      '',
    ].join('\n');
