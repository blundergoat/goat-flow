...
**Implements:** `ai-workflow-improvement-plan-prime.md` (Prime edition, v1.5)

> **Filename note:** The prompts reference `ai-workflow-improvement-plan-prime.md`. If your copy has a prefix or version suffix (e.g., `00-1-..._v1.4.md`), rename it to match before running the prompts.

## Changelog

| Version | Date       | Changes                                                                                                                                                                                                 |
| ------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| v1.5    | 2026-03-15 | Prompt B now lists sections (a)–(k) explicitly instead of cross-referencing Prompt A. Library skills expanded to 3–5 with cross-domain heuristic. Phase 1c: format hook skip guidance, pre-existing hooks migration, secret scanning to README. Footgun merge-with-existing path in both prompts. Phase 1a: guidelines-ownership-split.md as standard output |
| v1.4    | 2026-03-15 | Aligned with plan v1.4: permissions deny list, all 5 skills for all projects, /review naming conflict warning |
...
4. Rebuild CLAUDE.md with the execution loop. Target: under [120/100] lines.
   ...
   d) Default Execution Loop: READ → CLASSIFY → ACT → VERIFY → LOG
      - READ: read relevant files first, never fabricate codebase facts
        (include ❌/✅ example adapted for this project)
      - CLASSIFY: complexity and mode table. Include question vs directive
        disambiguation
      - ACT: behaviour per mode as a table. State declaration rule.
        Anti-planning-loop rule. Anti-BDUF guard with ❌/✅
...
5. code-review/SKILL.md — Structured review with RFC 2119 constraints.
   ⚠️ Do NOT name this skill "review" — it shadows Claude Code's
   built-in /review command. Always use "code-review".

[FOR LIBRARIES — create 3-5 skills:]
Create these skills under .claude/skills/:

ALWAYS create these 3:
...
CREATE IF the library has cross-domain complexity (multiple modules,
shared source files, cross-boundary coupling, multiple domains):
4. research/SKILL.md — Deep-read template adapted for the project's
   domain boundaries. Hard gate: no planning until human reviews.

CREATE IF the library has enough code surface for structured review
to add value over the default Review mode:
5. code-review/SKILL.md — Structured review with RFC 2119 constraints.
   ⚠️ Do NOT name this skill "review" — it shadows Claude Code's
   built-in /review command.

Single-domain libraries with flat structure: 3 skills is sufficient.
Multi-domain libraries or script collections: 4-5 skills.
