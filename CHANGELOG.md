...
`deny-dangerous` false-positive fixes - benign, read-only Bash that the guardrail wrongly blocked now passes, with no loss of protection against nested or obfuscated execution. Also clears stale Claude Code `MultiEdit` permission rules that warned on every launch.
