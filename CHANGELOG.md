...
- **Review/security artifact contract spike** - Added typed review/security artifact contracts, a validating security fixture, and a read-only Reviews dashboard stub that renders the fixture posture and findings. The source `goat-security` skill now documents `--emit json --output <path>` headless artifact behaviour, but authenticated `claude -p` and `codex exec` whole-repo probes did not emit an artifact within bounded attempts, so the headless producer is intentionally marked blocked rather than shipped as proven.
...
- **Shared hook policy store** - Shared guard logic now lives once per project in the committed `.goat-flow/hook-lib/` store (`patterns-shell.sh`, `patterns-paths.sh`, `patterns-writes.sh`, and `deny-dangerous-self-test.sh`). Fresh installs copy the per-agent dispatcher plus the store, chmod the scripts, and add `!hook-lib/` plus `!hook-lib/**` to `.goat-flow/.gitignore` so fresh clones keep the policy modules tracked.
...
- **Fail-closed hook startup and verification** - Missing or unreadable `.goat-flow/hook-lib/` now denies with an agent-shaped failure instead of falling through to exit 127. Audit and preflight check the dispatcher, the shared store, and registered guard-script paths for Claude, Codex, Antigravity, and Copilot.
- **Release-review guardrail gaps** - The 1.9.0 review caught two pre-tag blockers: `runConfiguredHookCommandSmoke` still runs `bash <extracted-script-path>` instead of the configured launcher string, so wrapper resolver, shell syntax, or executable-bit failures can pass audit; and `hooks enable` / `hooks sync` copies `.goat-flow/hook-lib/` without appending the installer's `!hook-lib/` `.goat-flow/.gitignore` exceptions, leaving required policy modules local-only in projects with old templates.
...
- **Three new on-demand playbooks** - `.goat-flow/skill-playbooks/observability.md` (logs / metrics / span events / trace context), `code-comments.md` (when and why to write a comment, portable across TS/Python/Go/Rust/shell), and `gruff-code-quality.md` (triage and verification for the gruff-* analyzer family).
...
- **Footgun: runner-vs-target agent confusion** - New entry in `.goat-flow/footguns/dashboard.md` documenting the `activeRunner` vs failing-agent conflation and the audit-scope mismatch, with real evidence from 2026-05-03 (a real downstream project) and three prevention rules.
