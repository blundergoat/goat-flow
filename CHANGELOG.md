...
- **Git guards block branch moves** - Deny soft resets, forced branch changes, fetch mappings to local branches, and stdin-fed fetch refspecs.
...
- **Git guards block more discards** - Deny directory restores across `cd` and `git -C`, forced `rm` and `checkout-index`, `read-tree -u --reset`, and forced worktree removal.
- **Policy hook crashes block work** - Unexpected child exits `1` and `127` deny in Claude, Antigravity, and Copilot without leaking partial output.
...
- **BREAKING: path audits require `audit`** - Replace `goat-flow <path>` with `goat-flow audit <path>`; bare `goat-flow` opens the menu, unknown commands exit `2`.
- **BREAKING: project-wide install baselines** - Clean `.goat-flow/install-state/<agent>.json` baselines merge into `.goat-flow/state/install/managed.json`; conflicts stop installation. Find repairs with `goat-flow status . --format json`.
- **BREAKING: state moves to `.goat-flow/state/`** - Stop and upgrade every writer, then run `goat-flow install . --agent <id>`: `.goat-flow/install-state/` moves to `state/install/`, `.goat-flow/write-claims/` to `state/locks/`. Unsafe state, outstanding claims or occupied destinations block migration. Old paths disappear; never run older versions afterward. Legacy installation refuses managed projects; use `goat-flow install <path> --agent <id>`.
- **BREAKING: stricter plan status and dependencies** - `plans check --strict` requires consistent tasks, proof and `Actual`, `M<digits>` filenames/titles, exact local dependency IDs, no cycles, completed prerequisites and at most one active milestone. `blocked`, `abandoned`, `superseded` and `deferred` require one `Status reason:`; other statuses omit it. Repair in-flight plans before `/goat-plan`; default checks are unchanged.
- **BREAKING: plain plan summaries** - `What problem are we solving` and `Who benefits and how` each require one plain 70–120-character sentence, excluding milestone IDs, ADR numbers, versions, flags and internal paths; legacy headings remain advisory.
- **BREAKING: `quality save` requires a refutation ledger** - Include `"refuted_candidates":[]`; shipped prompts do. Older reports remain readable by `validate`, `history` and `diff`.
...
- **BREAKING: one critique report contract** - Read stable finding IDs, rubric dimensions, host verification, source agent IDs and separate `HUMAN-PENDING` evidence. Replace `## Rubric Coverage Gaps` with `## Rubric Coverage`: dimensions are `finding`, `checked-clean` or `unassessed`. `CLEAN` permits limited coverage or lower-severity findings, never clearance.
- **BREAKING: revision-bound critique meta-audits** - Match `audited_revision` to `report_revision` and use current check names. One correction batch permits one recheck; later edits need remaining allowance or an unaudited label. Hosts cannot substitute their score; conformance does not prove correctness.
- **BREAKING: split writing playbooks** - Replace 1.16.0 `writing-style.md` instruction references with `writing-human-facing-prose.md` or `writing-agent-facing-instructions.md`; install preserves existing `writing-style.md` and `writing-for-agents.md` copies for review before removal.
- **Dedicated command help** - Every command's `--help` shows usage, subcommands, flags and examples without executing it.
- **Audits expose semantic drift and hook coverage** - `audit --check-content` compares documented states, limits, routes and inventories with shipped sources; audit, Markdown, dashboard and quality prompts report separate `hookCoverage.status`: `pass`, `warning` or `fail`.
- **Verify all hook groups** - `goat-flow hooks verify <path> --agent <id> --scenario all --trusted-target` retains every verdict and exits `1` unless all pass; JSON uses `goat-flow.hook-runtime-batch.v1`.
- **Retrieve and capture learning** - `goat-flow recall <path> [path...]` matches active evidence paths; `goat-flow learn new` validates entries and refreshes indexes; `--dry-run` previews. Indexes show dates and token cost; `stats` proposes graduation at two incidents (`Incident count` or recurrences). Lessons require an agent decision causing or mishandling failure.
- **Forecasts use narrower, measured ranges** - Before three measured milestones, `/goat-plan` and `plans check` use `1.0-2.5-6 min/unit`, replacing `0.5-2.5-10`. Below one minute/unit, advice uses measured low/high rates and a `1.00 min/unit` likely floor labelled `likely floored`; saved forecasts retain their values.
- **Scope growth stays visible** - `plans check`'s `unit growth:` separates registered additions in finished milestones from finished milestones whose checklist count changed without a saved forecast revision; unrecorded additions make this a floor.
- **Plans remain connected after scope changes** - New milestones link through the prior terminal milestone and recalculate `ISSUE.md` bands and totals. `superseded`/`deferred` milestones move estimates to `excluded:`, allow open checkboxes and paused receipts without `Actual`, and cannot be dependencies of active/complete work. Superseded reasons name resolvable successors; exports retain redacted reasons.
- **Completion reports retain proof** - The Proof Gate maps outcomes to evidence and gaps; milestones time active implementation, pause at human gates, finalize at exit and run `goat-clarity` once. `/goat-qa` plans retain risk maps and gap analysis.
- **Planning requests save files only when asked** - `/goat-plan` keeps plan and design requests inline until you ask for plan files; builds that `/goat` routes to planning still save the plan, start implementation and pause for your sign-off after each milestone.
- **Setup and skill qualification require evidence** - Setup accepts frontmatter `Use when` triggers; `goat-flow install . --dry-run` distinguishes exact-copy templates from generated/user-owned writes. Skill retirement requires repeated provider/model/config ablations with retained regression cases.
- **Git and GitHub protection preserves choices** - **Deny Git and GitHub writes** retains `deny-git-mutations` choices and read/comment exceptions; mixed-policy upgrades need separate dashboard Hooks consent, even with force options. Toggles and sync preserve provider order, quoted YAML siblings, anchors and saved commands. Missing Git choices stay on; upgrades record inheritance once. Disabling incompatible launchers requires reviewed Sync before saving; malformed policy-review data retains server diagnostics without an approval action.
- **Gruff follows project files and settings** - Claude uses the session project's install after directory changes; changed lines use the file's repository, non-Git files get whole-file analysis, Git failures report `git-scope-failed`. Nested `hooks.gruff-code-quality.enabled: false` and `binaries` overrides apply. `gruff-<language>.sh` wrappers in `bin/test/`, `bin/` or `scripts/` preserve project config, paths and reports; run `goat-flow hooks sync` once.
- **Gruff supports older analyzers** - `gruff.hook.v2` findings, warnings and failures work directly; older analyzers retain `gruff analyse`, supported `file` scope and stderr warnings alongside JSON. Refused configs, unreadable files and skips retain reasons. Docs, skipped folders, nested opt-outs and outside-session files stay quiet; deleted, renamed, binary and ignored source retains notices. Inside-session source outside the install reports `edited-path-outside-project`; `hooks verify` distinguishes quiet `non-source-edit` from source results. CLI and Gruff guidance explains selection and coverage.
- **Quality reports bind to the target** - Focused prompts use the target path and omit unrequested targets; failed audits add bounded learning context, refutations stay excluded, and new reports require per-axis evidence and deductions in `score_rationale`; historical reports remain readable.
- **Reviews retain evidence and boundaries** - Staged `/goat-review` runs create no Git tree objects; reports retain refuted IDs, bare JSON rows, gates, validator-ordered conclusions and `MUST`/`SHOULD`/`MAY` tags; project severity only ranks. Full diff and area reviews reach spec-drift and optional refuters, which declare enforced boundaries, forbid bypass flags and fall back to local-only review when unsupported. Bot matching strips one trailing `[bot]`, uses semantic locations and preserves unknown authors; three incident-based traps guard reasoning.
- **Critiques tie recommendations to evidence** - Recommendations and integration hooks cite surviving findings; blind spots and rankings need support, and early exits retain lower-severity findings. Only surviving recommendations reach approval. STRONG, ADEQUATE, WEAK and FLAWED rate the artifact separately from critic quality and coverage. Artifacts and rubrics work regardless of path; embedded instructions grant no reads, clean leak scans do not certify isolation, and leaks stop critics. Leaks and missing fields share one replacement allowance before incomplete coverage.
- **Clarity preserves intent, scope and compatibility** - Update/edit/fix grants documentation writes, report/review/check withholds them, then `documentation` applies. Approved private moves within one writable file need no repeat approval; cross-file/public changes retain gates and compatibility explanations. Inventory and applicable test-case accounting precede diagnosis; receipts separate assessed/protected files, retain exact formatter commands and baseline/final results, distinguish unavailable/skipped checks, and pause on ambiguous ownership or governing-rule drift.
- **Security separates findings from clearance** - Risks remain reportable with bound evidence, checked mitigations and supported severity; assessments justify threats and assumptions. Incomplete coverage stays `coverage-degraded` without clearance. Passive inspection supports explicitly trusted checkouts; hostile-checkout containment and concurrent-change exclusion remain unsupported. Verify every exception field, including finding class and compensating controls; absent controls need policy permission or findings stay `OPEN`. Posture is `block`, `needs-decision`, `accepted-risk`, `watch` or `none`; conclusion is `confident` or `coverage-degraded`; `tool-limited` marks degradation.
- **Security aligns reporting with verification** - Quick reports have eight sections; both depths load the same mandatory references; unavailable references degrade coverage. Authorized dependency audits gather leads before proof; execution-control gaps yield `execution-withheld`, approval-only gaps `scanner-withheld`; late leads return to verification. Gates cover all ten Git delta states.
- **Save approved security reports again** - Redaction uses fresh target-local `.goat-flow/logs/security/` paths and reports written, skipped or cleanup-needed outcomes; design text proves stated requirements, never deployed behavior.

### Fixed

- **Install failures explain recovery** - Concurrent writers, symlinked `.goat-flow`, unreadable hook policy and missing `js-yaml` stop before writes; dependency errors name the package root and `npm install`/reinstall, skipped hooks explain non-Git scan-root configuration. For stale claims, run `claims inspect <path> --target <project-relative-path>`, confirm no writer remains, then `claims recover --confirm-abandoned` with the exact marker digest; changed/unsafe markers remain untouched.
- **Recover legacy state before policy review** - Run `install --agent <id> --migrate-state-only` before dashboard review and installation; only bookkeeping moves, preserving hooks/policy choices. Unsafe state and outstanding claims still block.
- **Dashboard writes survive interruption** - Project identity, registry and active-plan selections use atomic replacement.
- **Claude/Copilot registrations no longer collide** - Claude preserves handler arguments; Copilot ignores cross-loaded rows and uses native hooks.
- **Codex evidence stays platform- and scenario-specific** - Windows launchers preserve cwd; upgrade from 1.16.0 with `goat-flow hooks sync .`, then restart Codex. Linux interactive CLI 0.155.1 evidence for both deny hooks expires 2026-10-21; fixed local scenario proof remains required. PostToolUse/Stop keep separate captures; Stop docs reflect approved 0.154.0 evidence and `scenario-unverified`.
