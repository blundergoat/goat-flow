...
- **Decision-record quality gate** - Scaffolded `.goat-flow/decisions/README.md` now gives agents a hard ADR filter, routing table, anti-pattern list, and mandatory pre-write checklist. `goat-flow stats --check` rejects malformed decision files: only `README.md` and `ADR-NNN-kebab-case-title.md` are allowed, and ADR files must include `**Status:**`, `**Date:**`, `## Context`, `## Decision`, plus a trade-off section (`## Consequences`, `## Failure Mode Comparison`, or `## Reversibility`). Failure messages point notes back to tasks, footguns, lessons, scratchpad, or the issue tracker instead of merely reporting a bad filename.
- **Decision metadata is advisory, not a release blocker** - ADRs missing recommended `**Author(s):**` or `**Ticket/Context:**` metadata now emit `decision-metadata` warnings while leaving `stats --check` green. The current repo still has historical ADRs without that metadata; those warnings are staged cleanup, not a hard v1.3.2 gate.
- **Non-destructive decisions README migration** - Fresh setup scaffolds the stricter decisions README, but existing projects keep customised `.goat-flow/decisions/README.md` files because the installer writes that template only when missing. Legacy non-ADR notes in `.goat-flow/decisions/` now fail `stats --check` with routing guidance until they are moved to tasks, scratchpad, lessons, footguns, or an issue tracker.
- **Dashboard audit fact profiles** - Dashboard summary audits now use a `dashboard-summary` fact profile that deliberately skips stack detection. Checks that need stack facts must declare `requiresStack`; regression coverage pins aggregate dashboard-summary audits so they do not call `detectStack` or accidentally read unavailable stack facts.
- **Dashboard audit caching and profiling** - `/api/audit?quality=true` now uses cache signatures over high-impact audit inputs and keeps the cache as a gitignored local artifact. `scripts/profile-dashboard-audit.mjs` is a supported maintainer profiler, exposed through `npm run profile:dashboard-audit` and documented in `.goat-flow/code-map.md`.
- **Filesystem glob performance path** - The audit filesystem now exposes `existsGlob()` for existence-only recursive checks, so stack detection can avoid materialising full glob result arrays for hot-path presence probes.
- **Release-readiness regressions** - v1.3.2 adds focused coverage for dashboard-summary no-stack behavior, dashboard audit cache cleanliness, existing-project decisions README compatibility, legacy decision-note failure guidance, and the installer round-trip/preflight drift fixture that previously caught the release-gate failure.
