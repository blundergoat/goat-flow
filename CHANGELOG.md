...
- **Windows post-turn scans finish in under a second** - Batched Git and pure Bash cut 25-file scans from 4m22.109s to 0.655s on Git Bash and 6.465s to 0.027s on Linux; 90 golden scenarios remain byte-identical.
- **Stop scans fail closed on macOS** - The 60-second `GOAT_FLOW_POST_TURN_SAFETY_MAX_SECONDS` budget blocks incomplete scans; Bash 3 matches newer Bash for credentials, expressions, and diff caps. Claude allows 90 seconds, and `goat-flow hooks sync` repairs drifted registrations.
- **Review validation and workflow guidance agree** - `review validate` preserves wrapped checklists, ignores standalone examples, rejects impossible coverage or verdicts, and accepts both Top-5 headings; plan, QA, review, glossary, footgun, and `plan.time` guidance match every mirror.
...
- **Milestone timing uses durable receipts** - `plans time start|stop|status` records UTC and epoch spans, permits one open span, finalizes at human review, marks discarded or reversed-clock spans incomplete, and rejects linked paths.
...
- **Unclaimed legacy receipts stay advisory** - Retrospective Actuals tolerate older hand-written receipt shapes; measured Actuals require valid shape and reconciliation.
...
- **Dashboard terminals preserve access mode** - Non-writing, unclassified, and investigator launches use `reporting`; write-enabled and direct sessions use `workspace`. The API defaults omissions to `workspace`, and retries preserve the resolved mode.
...
- **Claude assessments persist without shell access** - Claude writes one draft; the dashboard validates, saves, deletes, and receipts it while finalized reports stay read-only. See ADR-044.
- **Report capture stays private through recovery** - Invalid, wrong-owner, or oversized drafts are deleted and receipted immediately; concurrent saves remain distinct without accepting replacements, and capture ownership survives detach, retry, reconnect, and rehydration.
...
- **Test fixtures explain purpose and effects** - Plain-English comments close 104 documentation findings across 26 suites and make failures understandable without opening implementation files.
- **Contract failures name their surface** - Contract loops register cases by file or rule, so TAP identifies drift; tests rise from 1,370 to 1,408 with unchanged coverage.
...
- **Antigravity replaces Gemini CLI** - new third supported runner backed by `agy 1.0.1` (OAuth persistence verified on a fresh shell), sharing `AGENTS.md` and `.agents/skills/` with Codex; `--agent gemini` removed; dashboard composer-ready detection backed by a captured PTY fixture. See ADR-030.
