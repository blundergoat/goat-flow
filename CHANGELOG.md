...
- **Quality and setup prompt fixes** - Quality prompts no longer claim lean config requires `line-limits`; quality history IDs distinguish otherwise-similar findings; setup footgun status vocabulary now matches the linter-enforced `active | resolved` contract. (`src/cli/prompt/compose-quality.ts`, `src/cli/quality/ids.ts`, `workflow/setup/reference/footguns-readme.md`)
