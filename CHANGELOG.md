...
- **Dashboard harness percentage parity** - Dashboard report readers now preserve harness check `type` from `/api/audit`, so Home, Setup, and Quality per-agent percentages continue excluding metric checks like `test-runner-configured`; healthy agents display 100% instead of 94% while informational metric findings remain visible in the API. The learning loop records both the reader-field footgun and the card-vs-concern verification lesson, with a session log for the incident.
