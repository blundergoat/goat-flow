...
The gruff code-quality hook stops flagging files the project deliberately ignores, so coding agents are told an edited file is out of scope instead of being pushed to "fix" excluded code.

- **gruff hook honours `paths.ignore`** - The `gruff-code-quality` PostToolUse hook now reads the analyzer's own `ignoredPaths` / `paths.skipped` output: when an agent edits a file ignored by the project's `.gruff-*.yaml` config, the hook prints a single `skipped <path> - out of scope` line and surfaces no findings, instead of pushing the agent to "fix" a file the project excludes. The verdict comes from gruff itself - the hook never re-derives ignore rules, so there is a single source of truth - and the change lands in both the active `.claude/hooks/gruff-code-quality.sh` and the installed template `workflow/hooks/gruff-code-quality.sh` (kept byte-identical). It is forward-compatible: the gruff-* analyzers currently bypass `paths.ignore` for explicitly-passed file paths, so the hook is a safe no-op until they report ignored files in explicit/diff mode, at which point it activates automatically. A new `.goat-flow/lessons/gruff-cleanup.md` entry records the explicit-arg bypass so future ignore verification is scoped to a directory scan rather than a single named file.

## v1.9.0 - 2026-05-28

Worktree-safe hook launchers - Claude and Antigravity guardrails now run correctly inside `git worktree add` checkouts.

- **Single deny-dangerous hook** - the three split PreToolUse guardrails are consolidated back into one user-facing `deny-dangerous.sh` per agent, with shared policy modules and the central self-test committed under `.goat-flow/hook-lib/`. Fresh installs ship the dispatcher plus `hook-lib/`, un-ignore the store in `.goat-flow/.gitignore`, and expose exactly one "Deny dangerous hook" in the CLI/dashboard registry. Upgrades prune old split guard files, legacy `deny-dangerous.self-test.sh`, and stale registrations while preserving the new dispatcher and store.
