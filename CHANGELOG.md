...
**Audit System** - Replaced scanner/rubric engine (79 checks + 12 anti-patterns, point-based scoring) with audit system. 7 project-wide setup checks (config, directories, required files) are agent-agnostic. 9 per-agent checks (skills, versions, path integrity, hooks, deny patterns) run independently per agent. Advisory quality scoring (`--harness`) grades 5 concerns: context, constraints, verification, recovery, feedback loop. ADR-036.
