...
- **Generic credential-assignment scanning now requires env/config context and literal secrets** - `scan_env_assignment` no longer treats source-code expressions such as `tokens = re.findall(...)`, `token_count = len(items)`, `next_token = page["next_token"]`, `access_token = get_token()`, `tokenizer = build_tokenizer(cfg)`, `password_field = form["password"]`, or uppercase/Pascal-case config references such as `SETTINGS.API_TOKEN1` and `Config.DEFAULT_PASSWORD1` as hardcoded credentials. The generic assignment heuristic now runs only for env/config-shaped files such as `.env*`, shell scripts, YAML, TOML, INI, properties, and Dockerfiles; inside those files it classifies credential-shaped keys by key segments and suffix exclusions, then blocks only quoted string literals or bare credential-shaped tokens with enough entropy. Plural secret/password keys such as `CLIENT_SECRETS` and `DB_PASSWORDS` are scanned in those config surfaces, while interpolated double-quoted expressions such as `API_KEY="${PREFIX}..."` are not treated as literal hardcoded secrets. Provider-specific token detectors, private-key detection, and merge-conflict scanning are unchanged and still run everywhere.
- **Local data pipes into inline interpreters are allowed without reopening download-to-execute bypasses** - `deny-dangerous` now permits simple local data producers (`cat`, `printf`, `echo`) piped into visible inline interpreter code such as `node -e` or `python3 -c`, so read-only JSON/text inspection does not trip the "pipe to interpreter" rule. Raw interpreter execution (`cat script.py | python3`), network producers (`curl ... | python3 -c ...`), and shell-produced pipelines remain blocked, with clearer remediation copy.
