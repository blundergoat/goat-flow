...
Dashboard Skills polish, runner launch reliability, skill evaluator shape detection and report export, skill-quality config toggles, goat-security reference consolidation, and pre-ship correctness fixes.

- **Skills sidebar scope** - Dashboard Skills filters to real skills, drops shared references/playbooks from the sidebar and counts, resets stale selections, and updates copy from generic artifacts to installed skills.
- **Runner launch reliability** - Promptless backend session creation with browser-side send on readiness or a short after-output cap instead of the old 6s blanket fallback; pasted-text echo gate prevents Skills `Assess in Runner` from stalling at `[Pasted text #N]`; inline "Launching runner..." indicator via a new `install-invocation.ts` module centralising Windows bash discovery, runner-shim preference, and PATH probing.
- **Skill evaluator shape detection** - New `detectedShape`, `shapeConfidence`, and `shapeMismatch` for "packaged as skill, reads like playbook" cases; fallback-only subtype matches no longer report full confidence; browser/MCP commands count as tool dependencies; rubric stays portable for uploaded or non-goat-flow skills by crediting explicit context, gates, and evidence rules instead of preamble inheritance.
- **Skill evaluator clipboard report** - "Copy report" action emits a markdown summary mirroring the scoring engine with a 4s confirmation; `copyTextToClipboard` prefers `navigator.clipboard.writeText` and falls back to `execCommand("copy")` returning the result so the path surfaces a real "Copy failed" toast instead of silently claiming success.
- **Skill-quality config + scope toggles** - Workflow-completeness scoring reads its human-stop vocabulary from `quality-config`; new `scanDisk` gates disk-side reference resolution so uploads/fixtures opt out of filesystem probing without forking the scoring path; `prefetch` control suppresses stale dashboard prompts on artifact switch.
- **goat-security reference consolidation** - `auth-authz.md`, `dependency-and-supply-chain.md`, and `secrets-and-data-exposure.md` merged into `identity-and-data.md` and an expanded `supply-chain-and-cicd.md` across all four skill mirrors; task-workspace descriptions and ADR cross-references aligned.
- **Pre-ship correctness fixes** - Setup template Router Table splits skill-reference (meta) and skill-playbooks (tools); `docs/skills.md`, `.goat-flow/code-map.md`, and `workflow/setup/06-final-verification.md` aligned on canonical paths and the full 9-view list; `toShellProjectPath` preserves UNC roots (`//server/share`) that `path.posix.join` would otherwise collapse to local absolutes; launch-prompt fallback JSDoc documents the ungated cap as an intentional safety net for runners without readiness markers.
- **UI polish, gitignore, regression coverage** - Consistent max-widths and disabled-state styling across Setup/Quality/Projects/Skills with an inline SVG dropzone icon; the shipped `.gitignore` template exposes `.goat-flow/skill-playbooks/`; new unit coverage for Skills filtering, after-output cap, pasted-prompt fallback, copy-feedback timer, install-invocation candidate resolution, and `scanDisk` gating.
