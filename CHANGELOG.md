...
- **Preflight test runs have an internal timeout** - `scripts/preflight-checks.sh` wraps the test/coverage command in a process-group timeout (`GOAT_FLOW_PREFLIGHT_TEST_TIMEOUT_SECONDS`, default 600; `0` disables) and reports a timed-out test phase instead of buffering forever with no progress when a suite hangs. Retry runs use the same timeout.
