...
- **Default post-turn hook is universal safety, not project validation** - `post-turn-safety` now installs by default for Claude Stop events and scans changed content for obvious secrets, private keys, credential assignments, and merge conflict markers without requiring project toolchain setup. Codex and Antigravity are excluded because their Stop-hook delivery is unverified (Codex registered Stop hooks did not fire under codex exec 0.139.0; Antigravity hook trust gates execution and no Stop payload was captured firing).
