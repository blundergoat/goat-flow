...
- **BREAKING: path audits require `audit`** - Replace `goat-flow <path>` with `goat-flow audit <path>`; bare `goat-flow` opens the menu, and unknown commands exit `2`.
- **BREAKING: install baselines become project-wide** - Clean `.goat-flow/install-state/<agent>.json` baselines merge into `.goat-flow/state/install/managed.json`; conflicting baselines stop installation, and `goat-flow status . --format json` identifies repairs.
- **BREAKING: operational state moves to `.goat-flow/state/`** - Stop and upgrade every writer, then run `goat-flow install . --agent <id>` to move `.goat-flow/install-state/` to `state/install/` and `.goat-flow/write-claims/` to `state/locks/`. Unsafe state, outstanding claims or occupied destinations block migration; old paths are removed, and older versions must not run afterward. The legacy installer refuses managed projects; use `goat-flow install <path> --agent <id>`.
- **BREAKING: strict plan checks validate status and dependencies** - `plans check --strict` requires consistent tasks, proof and `Actual`, `M<digits>` filenames/titles, exact local dependency IDs, an acyclic graph, completed prerequisites and at most one active milestone. `blocked`, `abandoned`, `superseded` and `deferred` require one `Status reason:`; other statuses omit it. Fix in-flight plans before running `/goat-plan`; default checks are unchanged.
- **BREAKING: strict plan checks validate summaries** - `What problem are we solving` and `Who benefits and how` each require one plain 70–120-character sentence without milestone IDs, ADR numbers, versions, flags or internal paths; legacy headings remain advisory.
- **BREAKING: `quality save` requires a refutation ledger** - Add `"refuted_candidates":[]` before saving; shipped prompts include it, and older reports still open in `validate`, `history` and `diff`.
- **BREAKING: `redact --output` never overwrites** - Choose a fresh project-local path; existing files, linked parent directories and paths outside the project are refused.
- **BREAKING: critique reports share one contract** - Consumers must read stable finding IDs, rubric dimensions, host verification, source agent IDs and separate `HUMAN-PENDING` evidence; replace `## Rubric Coverage Gaps` with `## Rubric Coverage`, whose dimensions are `finding`, `checked-clean` or `unassessed`. Limited coverage or lower-severity findings can coexist with `CLEAN`; it never grants clearance.
- **BREAKING: critique meta-audits bind to a report revision** - Bind `audited_revision` to `report_revision` and use current check names; one correction batch permits one recheck, later edits need the remaining allowance or an unaudited label, and hosts cannot substitute their own score; conformance does not prove artifact correctness.
- **BREAKING: writing guidance uses separate playbooks** - Replace 1.16.0 instruction references to `writing-style.md` with `writing-human-facing-prose.md` or `writing-agent-facing-instructions.md`; install preserves existing `writing-style.md` and `writing-for-agents.md` copies for local review before removal.
- **Every command has dedicated `--help`** - Usage, subcommands, flags and examples display without executing the command.
- **Audits expose semantic drift and hook coverage** - `audit --check-content` compares documented states, limits, routes and inventories with shipped sources; audit, Markdown, dashboard and quality prompts report `hookCoverage.status` as `pass`, `warning` or `fail` separately from the top-level verdict.
- **Hook verification covers every group** - `goat-flow hooks verify <path> --agent <id> --scenario all --trusted-target` retains every verdict and exits `1` unless all groups pass; JSON uses `goat-flow.hook-runtime-batch.v1`.
- **Learning tools retrieve and scaffold entries** - `goat-flow recall <path> [path...]` matches active entries by evidence paths; `goat-flow learn new` previews with `--dry-run`, validates entries and refreshes indexes.
- **Learning-loop guidance puts prevention first** - Indexes show dates and token cost; `stats` lists graduation candidates at two incidents, from `Incident count` or recurrence labels, while lesson capture requires an agent decision that caused or mishandled a failure.
- **New plans use a narrower forecast range** - Until three measured milestones exist, `/goat-plan` and `plans check` use `1.0-2.5-6 min/unit` instead of `0.5-2.5-10`; saved forecasts remain valid.
- **`plans check` exposes scope growth** - Its `unit growth:` line separates units registered in finished milestones' forecast revisions from finished milestones whose checklist count differs without a saved revision; unregistered additions leave no trace, so the result is a floor.
- **Fast plans use their measured range** - Below one minute per work unit, `plans check` advises measured low/high rates with likely floored at `1.00 min/unit` and labelled `likely floored`; saved forecasts retain their numbers.
- **Plans stay connected through scope changes** - New milestones link through the prior terminal milestone and recalculate `ISSUE.md` bands/totals; `superseded` and `deferred` leave estimates on the `excluded:` line, allow open checkboxes and paused receipts without `Actual`, and cannot be dependencies of active or complete work. Superseded reasons name a resolvable successor; exports retain redacted reasons.
- **Skill completion reports retain their proof** - The Proof Gate maps each requested outcome to evidence and remaining gaps; milestone work times active implementation, pauses at human gates, finalizes at exit and runs `goat-clarity` once, while `/goat-qa` test plans retain their risk map and gap analysis.
- **Setup and skill qualification use explicit evidence** - Setup accepts frontmatter `Use when` triggers and distinguishes exact-copy templates from generated/user-owned writes through `goat-flow install . --dry-run`; skill retirement requires repeated provider/model/config ablations with retained regression cases.
- **GitHub writes share the Git switch** - **Deny Git and GitHub writes** retains `deny-git-mutations` choices and read/comment exceptions; mixed-policy upgrades require separate consent on the dashboard Hooks page, including when CLI or installer force options are used.
- **Hook toggles preserve configuration** - Enable, disable and sync retain provider order, quoted YAML siblings, anchors and saved commands; a missing Git choice stays on, upgrades record inheritance once, and disabling incompatible launchers requires reviewed Sync before saving. Malformed policy-review data retains the server diagnostic without an approval action.
- **Gruff follows the edited file across projects** - Claude feedback uses the session project's install after shell directory changes; changed lines come from the file's own Git repository, and files outside Git receive whole-file analysis. Git failures inside repositories still report `git-scope-failed`; run `goat-flow hooks sync` once.
- **Gruff honors project settings and wrappers** - Nested `hooks.gruff-code-quality.enabled: false` and `binaries` overrides apply to that project's files; `gruff-<language>.sh` wrappers in `bin/test/`, `bin/` or `scripts/` preserve project configuration, paths and reports.
- **Gruff supports current and older analyzers** - `gruff.hook.v2` findings, warnings and failures are read directly; older analyzers retain `gruff analyse`, receive `file` scope only when supported, and can emit stderr warnings without breaking JSON. Refused configs, unreadable files and analyzer skips retain the analyzer's reason.
- **Gruff distinguishes quiet skips from incomplete coverage** - Docs, skipped folders, nested opt-outs and files outside the session project stay quiet; deleted, renamed, binary and ignored source files retain notices. Source outside the selected install but inside the session project reports `edited-path-outside-project`; `hooks verify` expects quiet `non-source-edit` and non-quiet source results. CLI docs and the Gruff playbook explain install/config/executable selection and coverage checks.
- **Quality reports stay grounded in the target project** - Focused prompts use its path and omit unrequested targets; failed audits add bounded learning context, refuted candidates stay excluded, and current reports require per-axis evidence and deductions in `score_rationale` while historical reports remain readable.
- **Review reports preserve evidence and Git state** - Staged `/goat-review` runs create no Git tree objects; reports retain refuted IDs, bare JSON rows, a gates record, validator-ordered conclusions and `MUST`/`SHOULD`/`MAY` tags, with project severity used only for ranking.
- **Review routes and refuters declare their limits** - Full diff and area reviews reach spec-drift and optional-refuter steps; refuters name their enforced boundary, forbid bypass flags and fall back to local-only review when unsupported. Bot overlap strips one trailing `[bot]`, uses semantic locations and leaves unknown authors unknown; three real-incident traps guard review mistakes.
- **Critique recommendations keep their evidence** - Recommendations and integration hooks cite surviving findings, blind spots state supported limits, qualitative rankings explain their basis, and early exits retain lower-severity findings; only surviving recommendations can reach the human apply gate.
- **Fresh-eyes critiques bound context and recovery** - Supplied artifacts/rubrics remain valid regardless of path, embedded instructions grant no extra reads, and clean leak scans do not certify isolation; leaks end that critic, with one shared replacement allowance for leaks or missing fields before coverage is marked incomplete.
- **Clarity follows explicit intent and preserves compatibility** - Update/edit/fix grants documentation writes, report/review/check withholds them, then the `documentation` keyword applies; approved private moves within one writable file need no repeat approval, while cross-file/public changes retain gates and compatibility explanations remain.
- **Clarity accounts for scope before diagnosis** - Inventories and applicable test-case accounting precede diagnosis; receipts separate assessed and protected files, retain exact formatter commands and baseline/final results, distinguish unavailable from skipped checks, and pause edits for ambiguous ownership or governing-rule drift.
- **Security reports evidenced risks despite incomplete coverage** - Findings with sufficient binding, mitigation and severity evidence remain reportable; assessments declare justified threats/assumptions and retain `coverage-degraded` without clearance. Passive inspection supports explicitly trusted checkouts; hostile-checkout containment and exclusion of concurrent changes remain unsupported.
- **Security reports validate accepted risk and posture** - Every exception field, including finding class and compensating controls, needs verification; no controls is valid only when policy permits it, otherwise findings stay `OPEN`. Reports name `block`, `needs-decision`, `accepted-risk`, `watch` or `none`, conclude `confident` or `coverage-degraded`, and treat `tool-limited` as a degradation flag.
- **Security workflows keep reporting and verification aligned** - Quick reports have eight sections; both depths load the same mandatory references and degrade coverage when required references are unavailable. Authorized dependency audits gather leads before proof; execution-control gaps yield `execution-withheld`, approval-only gaps `scanner-withheld`, and late leads return to verification. Shared gates cover all ten Git delta states.
- **Approved security reports can be saved again** - Redaction writes to a fresh target-local `.goat-flow/logs/security/` path, and reports state written, skipped or cleanup-needed outcomes; design text proves stated requirements, never deployed behavior.

### Fixed

- **Installations stop competing or interrupted writers safely** - Concurrent installs stop before writes; inspect stale claims with `claims inspect <path> --target <project-relative-path>`, confirm no writer remains, then pass the exact marker digest to `claims recover --confirm-abandoned`; changed or unsafe markers remain untouched.
- **Legacy-state recovery avoids the policy-review loop** - Run `install --agent <id> --migrate-state-only` before dashboard review and normal installation; bookkeeping moves without changing hooks or policy choices, while unsafe state and outstanding claims still block.
- **Dashboard state survives interrupted writes** - Project identity, registry and active-plan selections use atomic replacement.
- **Claude and Copilot hook registrations no longer collide** - Claude handlers preserve arguments, while Copilot ignores cross-loaded rows and runs its native hooks.
- **Codex hook evidence distinguishes platform and scenario proof** - Windows launchers preserve the working directory; upgrade from 1.16.0 with `goat-flow hooks sync .` and restart Codex. Interactive Linux CLI 0.155.1 now has renewed live-delivery evidence for both deny policies through 2026-10-21; each still requires its fixed local scenario proof. PostToolUse and Stop retain their separate dated captures.
- **Codex Stop documentation matches current evidence** - The hook matrix and CLI guide now describe the approved interactive 0.154.0 capture and its `scenario-unverified` gate instead of the superseded stale state.
- **Preflight uses the stable test suite by default** - Preflight selects `test:fast` when available, keeping Node's intermittent experimental coverage reporter out of the default gate. Run `npm run test:coverage` separately; failing test commands still fail preflight.
- **Blocked secret access gives a safe next step** - Denials direct agents to checked-in examples or sanitized fields.
- **Critique assessment bands rate the reviewed artifact** - STRONG, ADEQUATE, WEAK, and FLAWED now have explicit criteria, separate from critic quality and coverage limits.
- **Install refusals explain actionable causes** - Symlinked `.goat-flow`, hook-policy read failures and missing `js-yaml` fail before project writes; dependency errors name the package root and required `npm install` or reinstall, and skipped hooks explain non-Git scan-root configuration.
- **Preflight reports failures and bounds registry waits** - Failed Tests rows include the first assertion/error, capped at 30 lines of 200 bytes; dependency audits fail after 120 seconds by default. `GOAT_FLOW_PREFLIGHT_AUDIT_TIMEOUT_SECONDS=N` changes the deadline; `0` disables only the timeout, leaving audits blocking.
- **Browser guidance follows the installed interface** - Check `browser-use --help` before choosing Python-stdin or legacy commands; guidance covers `browser-use skill`, bounded waits, fixture-login safeguards and tab recovery.

### Security

- **Learning reports cannot overwrite learning content** - `learn new --output` rejects learning-storage destinations and linked report files before publication, including dry runs.
- **Shell guards inspect executable syntax precisely** - Every pipeline stage's shell `eval` is checked; background commands and direct lockfile redirects are blocked, while `yq eval`, quoted evidence, arrows and escapes stay usable.
- **Git guards deny aliased writes and destructive flags** - Temporary/saved aliases expanding to `commit`, `reset --hard`, `clean -f` or `--no-verify` deny through quoting and escapes; read-only aliases remain allowed.
- **Interpreter guards cover more execution forms** - Perl, Ruby, Python, PHP and Deno checks deny paren-less process calls, `qx`/`%x`, pipe-open, `passthru`, `proc_open`, `pty.spawn` and `Deno.Command`; quoted operator text and JavaScript regex `.exec()` remain allowed.
...
- **Trusted CLI audits retain runtime hook proof** - `audit --agent <id>` runs configured hooks; `--untrusted-target` keeps it static.
- **Managed hook launches reject redirected scripts** - Hooks fail closed on symlinks, non-files, extra hard links, or root escapes.
- **Post-turn scans catch terminated credentials** - Native and Bash 3 scanners detect `KEY="value";` without scanning later expressions.
- **Managed hook names must match exactly** - Setup, drift, and smoke checks no longer mistake `custom-post-turn-safety.sh` for a managed hook.
- **Review parsing follows CommonMark** - Closing ATX markers and indented headings can no longer hide or invent review state.
- **Preflight and CI ratchet Gruff debt** - Stable IDs make new, growing, worsened, stale, or uncovered warning debt fail both gates.
- **Accepted Gruff debt stays visible** - Green runs list accepted warning identities, occurrences, coverage, and advisory totals.
...
- **Commit guidance gets a clearer filename** - New installs use `git-commit-message.md`; the legacy filename remains valid. See ADR-051.
...
- **Removed hook cleanup also runs on direct toggles** - `goat-flow hooks enable|disable <hook>` and dashboard single-hook toggles now prune stale removed-hook tombstones before applying the requested hook state, matching `hooks sync`. Upgrading from 1.12.0 no longer leaves old `plan-checkbox-guard` launcher entries or config blocks behind when an operator only toggles `post-turn-safety`.
- **Credential-assignment scanning is scoped to env/config surfaces** - The generic assignment heuristic now runs only for env/config-shaped files such as `.env*`, shell scripts, YAML, TOML, INI, properties, and Dockerfiles, and no longer treats uppercase/Pascal-case config references such as `SETTINGS.API_TOKEN1` and `Config.DEFAULT_PASSWORD1` as hardcoded credentials. Camel/Pascal-case keys such as `clientSecret`, `authToken`, and `refreshToken` normalize to the same key-segment rules without restoring broad substring matching. Dockerfiles cover both equals and space-separated `ARG`/`ENV` forms, including multi-assignment `ENV SAFE=x API_TOKEN=...` instructions. Plural secret/password keys such as `CLIENT_SECRETS` and `DB_PASSWORDS` are scanned in those config surfaces, while interpolated double-quoted expressions such as `API_KEY="${PREFIX}..."` are not treated as literal hardcoded secrets. Provider-specific token detectors and private-key detection are unchanged and still run everywhere.
- **`post-turn-safety` quoted credential false positives reduced** - Quoted credential values now skip config references and template interpolations such as Symfony `%env(...)%` / `%parameter%`, shell `${VAR}` / `$(...)`, Twig/Jinja/Helm delimiters, and ERB `<% ... %>` forms instead of treating them as hardcoded secrets. Quoted values containing whitespace are treated as prose, so translation strings like `password: 'Please enter your password'` and `token: 'Your verification token has expired'` no longer wedge Stop hooks. Opaque quoted tokens with no spaces and no template syntax still block. Merge-conflict detection now requires the real ordered `<<<<<<<` / `=======` / `>>>>>>>` triple in one file, so Markdown setext headings such as `Section` followed by `=======` pass. Lines with explicit allow markers such as `goat-flow-allow-secret` can carry intentional fixtures without disabling the hook.
- **Local data pipes into inline interpreters are allowed without reopening download-to-execute bypasses** - `deny-dangerous` now permits simple local data producers (`cat`, `printf`, `echo`) piped into visible inline interpreter code such as `node -e` or `python3 -c`, so read-only JSON/text inspection does not trip the "pipe to interpreter" rule. Raw interpreter execution (`cat script.py | python3`), direct network producers (`curl ... | python3 -c ...`), and filtered downloader pipelines such as `curl ... | cat | python3 -c ...` remain blocked, with clearer remediation copy.
- **Preflight test runs have an internal timeout** - `scripts/preflight-checks.sh` wraps the test/coverage command in a process-group timeout (`GOAT_FLOW_PREFLIGHT_TEST_TIMEOUT_SECONDS`, default 600; `0` disables), flushes captured output before reporting the child exit status, and reports a timed-out test phase instead of depending on an outer harness kill when a suite hangs. Retry runs use the same timeout.
- **Upgrade cleanup and generated indexes are more exact** - The standalone installer compacts removed `plan-guard` config blocks correctly in CRLF files, config block removal ignores unsafe internal keys instead of building a dynamic regex from them, and generated decision indexes can display a dedicated `**Superseded:**` date without overwriting an ADR's original acceptance date.
- **Dashboard learning-loop health shows without a perfect setup score** - The Home learning-loop panel, LEARNING LOOP pill, and regenerate-index button were gated on every setup check passing, so a single failing check (for example a missing post-turn hook) hid real learning-loop data behind "Setup first". They now show footgun/lesson counts, index freshness, and index regeneration whenever learning-loop records exist; the NA state appears only when no records exist yet and points at setup as what creates them.
- **`goat-flow stats` surfaces feedback-loop graduation candidates** - Active footgun/lesson entries carrying a line-start `**Recurrence update` marker (the recorded mistake happened again after the entry was written) are listed in the stats report with per-entry recurrence counts, as a prompt to promote the prevention to a structural gate (preflight check, CI step, deny pattern) or resolve the entry. Report-only by design: candidates never appear in or fail `stats --check`, resolved entries are skipped, and a corpus without recurrence markers renders no extra output.
- **CI gates learning-loop hygiene** - The CI workflow now runs `stats --check` after the audit step, so stale generated indexes, stale evidence refs, oversized buckets, and malformed ADRs fail pull requests instead of surfacing only in local preflight runs (which already gated this via the Learning-Loop Schema section). A new root `README_LOOPS.md` documents every feedback loop in the repo - trigger, closing mechanism, and where each one runs.
- **`gruff-code-quality` analyzer binaries can be configured per repo** - Monorepos whose analyzer lives outside the standard install locations (for example a nested `strands_agents/.venv/bin/gruff-py`) no longer need every agent session to export `GRUFF_PY_BIN`: a committed `hooks.gruff-code-quality.binaries.<lang>` entry in `.goat-flow/config.yaml` names the executable once for all agents. Config values must be repo-relative, resolve inside the repo, and point at an executable file; the `GRUFF_<LANG>_BIN` env override still wins, arbitrary nested venvs are still never auto-discovered (ADR-032), and an override that is set but wrong prints a specific diagnostic (non-existent, non-executable, or non-repo-relative path) instead of silently falling back to discovery. Dashboard hook toggles and `hooks sync` preserve the `binaries` block instead of rewriting it away, the missing-binary diagnostic now names the config key alongside the env var, and the hook self-test covers config acceptance, env-beats-config precedence, quoted/CRLF config parsing, and each rejection diagnostic.
- **Orientation docs match live dashboard and audit behaviour** - Architecture/code-map route inventories now include `dashboard-index-routes.ts`, and frontend coding standards now state that `audit --harness` failures contribute to the overall audit status and exit code.
- **Compact guidance, preflight timing, and settings-glob traps are consistent** - Instruction files and the shared skill conventions now state the same compact trigger (~60% context or after 15+ turns), preflight section timing clamps negative wall-clock deltas so a PASS run cannot render `-1.0s`, and a new `agent-settings` footgun records that settings-layer deny globs match guarded phrases quoted inside benign read-only commands while the deny hook itself correctly allows them.
...
- **`goat-flow audit --untrusted-target` skips executing the target's hook code** - The deny-mechanism runtime smoke runs the audited checkout's own configured launcher and managed hook script, which is unsafe for a checkout you don't trust. The new `--untrusted-target` flag downgrades that to static deny checks only; the default audit is unchanged.
- **Codex Workspace terminals can run preflight** - Dashboard-launched Codex sessions now start with `--sandbox danger-full-access`, so `scripts/preflight-checks.sh` can run nested Node/git/npm subprocesses and npm registry checks instead of failing with sandbox `EPERM` and DNS errors.
...
- **`goat` dispatcher routes ambiguous requests more concretely** - The dispatcher now records `target-files=unknown` when no files are named and gives Route Snapshot examples for explicit skill calls, symptoms, auth reviews, and ordered multi-intent requests.
- **`goat-plan` handoffs are more executable** - The planning skill now includes concrete Path-Only Intake and File-Write examples with expected plan artifacts, `.active` one-line pointer behavior, no-write outcomes, and stakeholder-facing `ISSUE.md` guidance instead of placeholders.
- **`goat-debug` is clearer at the first fork** - The debugging skill now gives deterministic Quick vs Full depth criteria, requires D1.5 minimisation before any quick-path D2 root-cause claim unless the reproduction is already minimal, and includes a portable worked D1-to-D2 example.
- **`goat-critique` examples and fresh-eyes rules are more concrete** - The skill now documents Agent C as a fresh-eyes control group with schema-compatible lens fields but no per-lens quota, keeps `## Outcomes` out of the initial Phase 5 gate response until the human chooses A/B/C/D, and replaces synthetic rubric examples with real goat-flow evidence plus a Phase 2 context-leak walkthrough.
- **`goat-security` report calibration is clearer** - The skill now includes a concrete confirmed-finding example, a false-positive removal example, and sharper guidance that `project-policy-template.md` is loaded only when creating or revising `.goat-flow/security-policy.md`, not during normal security reviews. The source template and all installed mirrors stay aligned.
- **`goat-review` reports review integrity more explicitly** - The skill now gives systemic patterns a dedicated output slot, mirrors automated-review and cross-model refuter summaries in Review Integrity, carries the matching degradation flags, and includes concrete refutation and finding-format examples across all installed mirrors.
- **`goat-qa` gives testers a concrete gap-analysis handoff** - The skill now includes a worked Standard Mode example, spells out Regression Guard handoff steps, and says Audit Mode creates no plan file unless separately approved.
...
Directory migration release: goat-flow now installs a coherent `.goat-flow/` layout with plans, learning-loop memory, central hooks, and grouped skill docs. Also includes `deny-dangerous` false-positive fixes and clears stale Claude Code `MultiEdit` permission rules that warned on every launch.

- **`.goat-flow/` folder layout migrated in one upgrade path** - Fresh installs now use `.goat-flow/plans/` instead of `.goat-flow/tasks/`, `.goat-flow/learning-loop/{decisions,footguns,lessons,patterns}/` instead of top-level learning-loop buckets, `.goat-flow/hooks/` instead of `hook-lib/` plus fat per-agent hook copies, and `.goat-flow/skill-docs/` for shared skill doctrine and playbooks. Upgrade migration preserves `.goat-flow/plans/.active`, existing local milestone subdirs, custom config path overrides, user-authored learning-loop entries, and same-named collision files by leaving old entries in place rather than overwriting new content. Dashboard plan APIs, the deprecated `/api/tasks` alias, `goat-plan` routing, setup templates, audit guidance, and hook sync now point at the new layout.

- **Command substitution with control operators no longer over-blocks** - A read-only `$(...)` containing `&&`, `||`, `;`, or a newline (e.g. `v=$(grep -m1 version "$f" 2>/dev/null || echo MISSING)` inside a `for` loop) was wrongly denied with `Policy destructive: Complex command substitution`. The command-segment splitter tracked quotes but not parenthesis depth, so an operator inside an unquoted substitution split `$( ... )` across two segments and left an orphan `$(` that the residual catch-all blocked. The splitter is now command/process-substitution aware (`$(`, `<(`, `>(` open a no-split region); operators inside a substitution are still policy-checked by recursing into the interior, and plain `( ... )` subshells stay splittable so `(cmd && rm -rf /)` cannot bypass the guard. Dangerous substitutions (`$(... | bash)`, backtick substitution, `$(true || rm -rf /)`, `$(git push ...)`) still block - and now report the accurate reason (e.g. `rm -r without safe scoping`) instead of the generic substitution message. The chained-segment cap is enforced at nested substitution depths too. (Deeply nested but benign substitutions such as `echo $(dirname $(dirname $(dirname $(pwd))))` are allowed - each interior segment is still policy-checked, so depth itself is not blocked.)
- **Arithmetic expansion is no longer mistaken for command substitution** - `echo $((1 + 2))`, `n=$((COUNT + 1))`, `$(( (1 + 2) * 3 ))`, and nested forms like `$(( $(( 1 )) ))` were denied with the same `Complex command substitution` message because the scanner only recognised `$( )`. Arithmetic `$(( ))` is now masked before the catch-all; a command substitution nested inside arithmetic is still stripped and policy-checked first, so `$(( $(rm -rf /) ))` still blocks.
- **Reading `.env.example` with a redirect is no longer treated as a write** - `ls -la .env.example 2>&1` and `cat .env.example 2>/dev/null` were denied as `.env.example ... read-only inspection only`, because any redirect on a command touching `.env.example` was classified as a write. Only a redirect whose target is `.env.example` itself now counts as a write; a stderr dup, a stderr discard, or a redirect to another file stays a read. Writes to `.env.example` still block - bare, clobber, append, and path-prefixed targets alike (`echo X > .env.example`, `printf x >.env.example`, `... >> .env.example`, `> ./.env.example`, `> sub/.env.example`) - as does all real secret-file access (`.env`, SSH/AWS/GCP keys, credentials). A broad `*.env*` glob was never blocked on its own.
- **Self-test corpus closed the gap** - The central `deny-dangerous` self-test now exercises every false-positive class above plus its dangerous counterpart on both sides; smoke and full modes stay green. New `.goat-flow/learning-loop/footguns/deny-dangerous.md` (a dedicated footgun bucket split out of the oversized `hooks.md`) and `.goat-flow/learning-loop/lessons/hook-testing.md` entries record the splitter parenthesis-depth trap and the `.env.example` redirect-classification trap. The change lands through the central `.goat-flow/hooks/deny-dangerous.sh` dispatcher and `.goat-flow/hooks/deny-dangerous/` policy modules, with workflow templates kept in sync. Existing projects pick up the fix by re-running `goat-flow setup` or `goat-flow hooks sync`.
- **Stale `MultiEdit` permission rules removed (Claude Code v2.x)** - Claude Code removed the `MultiEdit` tool, so the shipped `.claude/settings.json` printed `Permission deny rule "MultiEdit(**/.env*)" matches no known tool` warnings on every launch. The 13 redundant `MultiEdit(...)` secret-path deny rules are dropped from the Claude template and installed settings - every path keeps its `Read`/`Edit`/`Write` deny, so secret-path coverage is unchanged - and the dead `MultiEdit` gruff PostToolUse matcher is removed from `src/cli/server/hooks-registry.ts`, `.claude/settings.json`, and `.codex/hooks.json` (now `Edit`/`Write`). `workflow/hooks/README.md` and the gruff hook self-test fixture updated to match. Existing projects pick this up via `goat-flow hooks sync`.
- **Home setup CTA launches the generated setup prompt** - The Home "Fix with <runner>" setup action now fetches the same `/api/setup` output as the Setup page and launches that generated prompt through the selected runner, instead of pasting the literal `goat-flow setup . --agent <runner>` command. The Home preview copy now labels the generated setup prompt honestly, stale unused Home prompt builders were removed, and focused regressions cover generated setup launch, same-project request supersession, setup-generation failure handling without duplicate toasts, and harness runner-vs-target separation.
- **Review/security artifact contract spike** - Added typed review/security artifact contracts, a validating security fixture, and a read-only Reviews dashboard stub that renders the fixture posture and findings. The `goat-security` skill now documents `--emit json --output <path>` headless artifact behaviour across the workflow template and installed skill mirrors. Claude proved the security-target producer path by writing a contract-valid `SecurityResult` artifact after continuing with `claude -p -c --permission-mode acceptEdits`; the plain `claude -p` scan reached a write-permission prompt, so the 1.10.0 job runner must encode that non-interactive permission requirement. `codex exec` parity remains a recorded timeout gap.
...
- **Submodule and bare-repo launcher hardening** - Agent launcher templates distinguish linked worktree gitdirs from absorbed submodule gitdirs and guard every `git rev-parse --show-toplevel` fallback. A missing worktree root now exits through the policy hook's deny path instead of falling through as an unguarded command failure.
- **Hook launcher no longer wedges sessions that leave the repo** - The PreToolUse launcher resolved its guard path with `git rev-parse` run in the agent's current working directory and failed closed when that directory was outside any git repo. A session that `cd`'d into `/tmp` (e.g. to stage scratch files) then had every Bash blocked with `BLOCKED: Policy hook unavailable: git repository root unavailable.` - including the `cd` back into the repo, since the guard runs before the command's own `cd` - leaving the shell permanently wedged while Read/Edit/Write still worked. The `--git-common-dir` worktree hardening did not help: it also fails from `/tmp`. Launchers in `src/cli/server/agent-hook-writer.ts`, `workflow/hooks/agent-config/claude.json`, `workflow/hooks/agent-config/antigravity-hooks.json`, and `workflow/install-goat-flow.sh` now fall back to the cwd-independent `$CLAUDE_PROJECT_DIR` (the project root Claude Code exports to hooks) before failing closed, and fail closed only when no `deny-dangerous.sh` is found by either the git root or that env var. They also `cd "$root"` before invoking the guard, because `deny-dangerous.sh` re-resolves its own `.goat-flow/hooks/deny-dangerous` root via `git rev-parse` from cwd - resolving only the script path left the guard failing closed from `/tmp` anyway; `cd` failure also fails closed. Git resolution still runs first, so worktree/submodule checkouts are unaffected; the guard still executes, so enforcement is unchanged. Only Claude exports `$CLAUDE_PROJECT_DIR`, so other agents still fail closed outside a repo until their own root env var is wired in. Existing projects re-run `goat-flow setup` or `goat-flow hooks sync` to pick up the new launcher. New `.goat-flow/learning-loop/footguns/hooks.md` and `.goat-flow/learning-loop/lessons/agent-behavior.md` entries record the trap and the `!cd <repo>` recovery.
- **Clearer hook failure copy** - Launcher startup failures now say `Policy hook unavailable` with the concrete root-resolution cause, deny responses identify the specific policy scope, and audit failures distinguish path mismatches from commands that exit before the managed hook script starts.
...
- **Codex permission profiles launch again** - Codex configs now extend the built-in `:workspace` permission profile and add secret-path `deny` rules, instead of rebuilding workspace access with the older `:workspace_roots`/`none` shape that made shell commands fail before startup with `bwrap: execvp .../vendor/.../codex: No such file or directory` on Codex 0.136. `goat-flow install . --agent codex` migrates preserved `.codex/config.toml` files in place and keeps custom deny entries as `deny`.
...
- **Single `deny-dangerous` guardrail hook** - The three split PreToolUse guardrails are consolidated back into one user-facing `deny-dangerous.sh` per agent. The CLI/dashboard hook registry now exposes exactly one required "Deny dangerous hook" instead of separate destructive-shell, secret-path, and repository-write entries, while keeping the same block/allow coverage for destructive shell commands, direct secret-path access, `git commit` / `git push`, destructive git flags, and GitHub CLI writes.
- **Shared hook policy store** - Shared guard logic now lives once per project in the committed `.goat-flow/hooks/deny-dangerous/` store (`patterns-shell.sh`, `patterns-paths.sh`, `patterns-writes.sh`, and `deny-dangerous-self-test.sh`). Fresh installs copy the dispatcher plus the store, chmod the scripts, and add `!hooks/` plus `!hooks/**` to `.goat-flow/.gitignore` so fresh clones keep the policy modules tracked.
- **Hook upgrade cleanup** - `goat-flow install` now prunes old per-agent `guard-common.sh`, `guard-destructive-shell.sh`, `guard-secret-paths.sh`, `guard-repository-writes.sh`, `guardrails-self-test.sh`, and legacy `deny-dangerous.self-test.sh` files from canonical agent hook directories. Existing hook config files are migrated in place: stale `guard-*` / old deny registrations are removed, the canonical `deny-dangerous.sh` registration is added, and `.goat-flow/config.yaml` aliases old guard hook toggles to `deny-dangerous`.
- **Fail-closed hook startup and verification** - Missing or unreadable `.goat-flow/hooks/deny-dangerous/` now denies with an agent-shaped failure instead of falling through to exit 127. Audit and preflight check the dispatcher, the shared store, and registered guard-script paths for Claude, Codex, Antigravity, and Copilot.
- **Release-review guardrail gaps** - The 1.9.0 review caught two pre-tag blockers: `runConfiguredHookCommandSmoke` still runs `bash <extracted-script-path>` instead of the configured launcher string, so wrapper resolver, shell syntax, or executable-bit failures can pass audit; and `hooks enable` / `hooks sync` copies `.goat-flow/hooks/deny-dangerous/` without appending the installer's `!hooks/` `.goat-flow/.gitignore` exceptions, leaving required policy modules local-only in projects with old templates.
...
- **Documented as a learning-loop footgun** - New entry in `.goat-flow/learning-loop/footguns/hooks.md` records the worktree symptom, the `--show-toplevel`-vs-`--git-common-dir` root cause, and the invariant any future launcher edit must satisfy. `workflow/hooks/README.md` is rewritten to describe the new resolution flow.
- **One commit-conventions file, IDE-readable** - commit guidance now lives only at `docs/coding-standards/git-commit.md`; the removed `.github/git-commit-instructions.md` was never auto-read by IDEs (VS Code and JetBrains read `.github/copilot-instructions.md`), so its rules went unseen. The auto-read instruction files now carry a `## Commit Messages` summary pointing to the canonical doc and add a branch-prefix rule (a `feat/<digits>` branch → `#<digits>` subject prefix). The `commit-guidance` harness check and the Copilot `agent-instruction` bridge now require the docs location plus the instruction-file reference, and `src/cli/prompt/commit-guidance.ts` seeds the doc on install. See ADR-051.
- **gruff hook honours `paths.ignore`** - The `gruff-code-quality` PostToolUse hook now reads the analyzer's own `ignoredPaths` / `paths.skipped` output: when an agent edits a file ignored by the project's `.gruff-*.yaml` config, the hook prints a single `skipped <path> - out of scope` line and surfaces no findings, instead of pushing the agent to "fix" a file the project excludes. The verdict comes from gruff itself - the hook never re-derives ignore rules, so there is a single source of truth - and the change lands in both the active `.goat-flow/hooks/gruff-code-quality.sh` and the installed template `workflow/hooks/gruff-code-quality.sh` (kept byte-identical). All five analyzers (`gruff-go`, `gruff-rs`, `gruff-ts`, `gruff-php`, `gruff-py`) now apply `paths.ignore` to explicitly-passed file paths and diff/changed-range modes (verified against the `gruff-workspace` checkouts on 2026-05-30; the earlier `gruff-ts` explicit-arg bypass is fixed), so this hook is active, not a no-op. A new `.goat-flow/learning-loop/lessons/gruff-cleanup.md` entry records the historical explicit-arg bypass.
- **gruff hook explains config-schema rejections** - When the project's `.gruff-*.yaml` is missing the `schemaVersion:` line that `gruff-rs`, `gruff-ts`, and `gruff-php` now require, `analyse` exits non-zero with an empty findings payload. The hook previously discarded gruff's error and printed a misleading `produced non-JSON output` note to stderr, so the agent never learned the cause. The `gruff-code-quality` hook now detects the schema rejection and relays gruff's own message - including its `<binary> init --force` remediation - to stdout, while never editing the project's gruff config itself. Applied identically across all five shipped hook copies (`.claude`, `workflow`, `.github`, `.codex`, `.agents`); covered by a new smoke-test case.
- **gruff-ts cleanup follow-up** - Low-risk modernisation, naming, comment, assertion-constant, and test-readability findings were fixed across CLI, dashboard, scripts, and tests while leaving disputed rule families as explicit accepted debt instead of weakening gruff configuration. The full `gruff-ts analyse --fail-on none` snapshot now reports 428 findings, down from the 717-finding baseline for this pass, with `error=0`.
- **gruff-ts size cleanup** - The remaining `size` findings were fixed by splitting oversized CLI, audit, quality, server-route, dashboard, and test modules into focused files instead of raising analyzer thresholds. Dashboard classic-script assets now load through smaller fragments, terminal-launch tests moved into a subfolder suite, and long audit/quality/skill test filenames were shortened with grouped unit-test folders.
- **CI uses the split test contract** - GitHub Actions now runs `npm run test:full` instead of duplicating a raw `node --test` glob, so split audit-drift and dashboard integration suites use the same fast/serial-slow invocation as local verification. This keeps standalone test modules importing their helper scope and prevents stateful dashboard tests from racing each other.
- **Learning-loop index prototype** - 1.9.0 now includes a hand-built `.goat-flow/learning-loop/footguns/INDEX.md` prototype covering 69 active footguns with grep-friendly `(search: "...")` anchors. The spike measured 311 active/indexed learning-loop entries overall, rejected always-loaded index content as too large, chose per-bucket indexes plus mandatory Step 0 read, and moved the generator/freshness/emission work to `tasks/1.10.0/learning-loop/`.
...
- **Antigravity replaces Gemini CLI** - new third supported runner backed by `agy 1.0.1` (OAuth persistence verified on a fresh shell), sharing `AGENTS.md` and `.agents/skills/` with Codex; `--agent gemini` removed; dashboard composer-ready detection backed by a captured PTY fixture. See ADR-020.
- **Dashboard hook management + guardrail split** - new `/hooks` dashboard page and `goat-flow hooks <list|enable|disable|sync>` CLI manage shipped hooks across Claude, Codex, Antigravity, and Copilot via `.goat-flow/config.yaml`. The monolithic `deny-dangerous*` files are replaced by three togglable guardrails (`guard-destructive-shell.sh`, `guard-secret-paths.sh`, `guard-repository-writes.sh`) with one central `guardrails-self-test.sh`, plus an opt-in `gruff-code-quality.sh` PostToolUse hook that only surfaces findings on changed lines.
- **Proof class required on every finding** - `goat-critique`, `goat-qa`, and `goat-security` now tag every sub-agent finding, validated finding, and recommended change with one of `RUNTIME | CONTRACT-GREP | STATIC | NOT-REPRODUCED` alongside the existing OBSERVED/INFERRED/UNVERIFIED evidence quality; new contract tests pin the requirement.
- **Codex install migrates filesystem permissions** - `goat-flow install . --agent codex` rewrites the legacy `:project_roots` anchor to `:workspace_roots` and converts invalid glob `none` entries into the canonical shape Codex 0.131 accepts, with a post-install validator that aborts on any remaining unsupported form.
- **Dashboard awaiting-input badge: asymmetric trust** - after five rounds of output-driven heuristics failed, the badge is now SET from runner output but CLEARED only by input-side signals (keystrokes, programmatic sends, Ctrl+V paste, session lifecycle). New PTY fixtures cover Claude, Codex, Antigravity, and Copilot startup/running states.
- **Three new on-demand playbooks** - `.goat-flow/skill-docs/playbooks/observability.md` (logs / metrics / span events / trace context), `code-comments.md` (when and why to write a comment, portable across TS/Python/Go/Rust/shell), and `gruff-code-quality.md` (triage and verification for the gruff-* analyzer family).
- **GitHub CLI read-only by default (ADR-028)** - the deny hook now blocks `gh` write paths (issue/PR comments, releases, workflow runs, repo edits, gists, secrets, keys, `gh api` writes) while keeping `gh issue view`, `gh pr view/diff/checks`, `gh search`, `gh repo view`, and read-only `gh api` available.
- **Gruff-ts cleanup pass across ~80 files** - the new playbook applied to goat-flow's own TypeScript surface: JSDoc added to exported interfaces and functions, short identifiers renamed to domain terms across `src/cli/audit/`, `src/cli/server/`, `src/cli/quality/`, `src/cli/manifest/`, `src/cli/stats/`, `src/cli/prompt/`, `src/cli/detect/`, `src/cli/evidence/`, `src/cli/facts/`, and `src/dashboard/`. No runtime behaviour changes.
- **goat-review automated-reviewer overlap (PR mode)** - `/goat-review` in PR mode now ingests existing findings from common GitHub review bots and tags each Pass 2 finding `[overlap:<reviewer>]` or `[new]` via a same-file + Jaccard-token heuristic, with a new Review Integrity line summarising overlap and net-new counts.
- **ADR-029 two-ceiling runaway protection** - any orchestrated long-lived resource (PTY session, batch worker, scheduled remote agent) must ship with both a resource-layer ceiling and a consumer-loop ceiling, with the consumer ceiling firing strictly first so it can wind down through normal control flow before the resource is reaped.
- **Learning-loop expansion** - seven new footguns (`agent-output-trust.md`, `caching.md`, `cleanup-layering.md`, `config.md`, `internal-run-isolation.md`, `sentinels.md`, plus a `cli.md` extension), ten lessons (including new `external-pr-learnings.md` and a local-incident "never disable gruff-ts rules" rule), and eight patterns spanning a new `dependencies.md` plus appends to `architecture.md`, `refactoring.md`, and `verification.md`.
...
- **Regression coverage** - Added tests for Home readiness scoring, Setup target card scoring, full-scope setup prompt behavior, outdated prompt issue details, and `/api/setup` self-test avoidance. (`test/integration/dashboard-server.test.ts`, `test/unit/audit-command/main.test.ts`)
- **Verification lesson** - Recorded the lesson that behavior-scope changes need matching assertion updates before the first focused run, after an initial stale test assertion caught the old harness-only expectation. (`.goat-flow/learning-loop/lessons/verification.md`)
...
- **Footgun: runner-vs-target agent confusion** - New entry in `.goat-flow/learning-loop/footguns/dashboard.md` documenting the `activeRunner` vs failing-agent conflation and the audit-scope mismatch, with real evidence from 2026-05-03 (a real downstream project) and three prevention rules.
...
- **Structural reorg** - Patterns directory split into 5 categorised buckets. 5 skill-specific reference packs relocated from shared `skill-reference/` to per-skill `references/` subdirectories. DDT layer removed (ADR-023).
...
- **Learning-loop guidance** - `.goat-flow/learning-loop/patterns.md` now captures the rule behind the dashboard latency fix: summary surfaces should use cheap evidence, while drill-in routes keep the expensive runtime proofs.
...
- **footgun** - New `.goat-flow/learning-loop/footguns/cli.md`: "ESM main-module guard breaks under symlinks" documents the structural trap and prevention rules.
- **lesson** - `.goat-flow/learning-loop/lessons/verification.md`: "Test suite must exercise the published invocation path" - all 359 tests passed while the CLI was broken for every real consumer, because no test went through the `.bin/` symlink.
...
- **Instruction file retrieval** - All four agent instruction files now include `.goat-flow/learning-loop/decisions/` in retrieval for architecture/policy/setup tasks.
...
- **Ask First boundary** - `.goat-flow/skill-docs/` added to CLAUDE.md Ask First boundaries. Shared doctrine loaded by every skill now requires approval before edits.
...
- **Active plan semantics** - `.goat-flow/plans/.active` treated as an advisory local pointer, not a setup invariant. Missing/stale markers fall back to listing task dirs and asking. ADR-017, glossary, code-map, and quality prompt aligned.
...
- **Install + manifest anchors** - New `touch_anchor` helper in `workflow/install-goat-flow.sh` seeds `.goat-flow/learning-loop/decisions/.gitkeep` and `.goat-flow/logs/sessions/.gitkeep` on install so the learning-loop + continuity-note directories exist in fresh projects. `workflow/manifest.json` adds both anchors to required paths.
...
- **Docs + footguns** - `README.md` switches from `npx goat-flow` to `goat-flow` in usage examples (expects a global install). `docs/audit-and-quality.md` satellite-agents example corrected (`.gemini/skills/` → `.github/skills/`). New footgun in `.goat-flow/learning-loop/footguns/docs-and-crossrefs.md`: **Prose examples for agent-specific paths drift from the manifest** - name-based inference (`gemini` → `.gemini/skills/`) is wrong by default because codex and gemini both share `.agents/skills/`, and `doc-paths-resolve` only checks existence, so a plausible-but-wrong path that happens to exist passes the audit silently. Prevention: grep `workflow/manifest.json` before hand-writing agent-specific paths. `goat-plan` Excuse/Reality table drops the "team is experienced, spike is overkill" row.
- **Lesson + auditor footgun** (`1899366`) - Preflight's sanitization pipeline (`| grep -oE '^[0-9]+$' | tail -1`) could turn non-empty node output into empty output for the first time; `setup_count` was only assigned inside the `[[ -n "$build_count" ]]` branch but referenced unconditionally later, crashing under `set -u` with `unbound variable`. New lesson in `.goat-flow/learning-loop/lessons/agent-behavior.md` on tracing downstream references when adding output filters, and a matching auditor footgun update.

## v1.2.0 - 2026-04-20

Skill renames, Copilot CLI support, audit subsystem expansion (drift/content/manifest), proof-gate verification, structured learning loop, and a quality-report follow-up wave that closed a CRITICAL Bash secret-read gap. 324 files changed, 86 commits.
...
- **Learning loop** - `last_reviewed` frontmatter on footguns/lessons/decisions. New `goat-flow stats --check` with per-bucket freshness bands + search-anchor resolution. Active-plan marker `.goat-flow/plans/.active` disambiguates live vs archived plans. 6 glossary terms added.
...
- **Restructure** - `docs/lessons/`→`.goat-flow/learning-loop/lessons/`, `docs/decisions/`→`.goat-flow/learning-loop/decisions/`, `agent-evals/`→`.goat-flow/evals/`, `ai/instructions/`→`.goat-flow/coding-standards/`, `tasks/`→`.goat-flow/plans/` (gitignored)
