...
Skills are **created by Phase 1b** in the runtime setup prompts (`runtime/claude-code-prompts.md` or `runtime/codex-prompts.md`). This folder documents what each skill does and when it's needed.
...
|-------|---------|---------|--------|
| `/preflight` | Pre-task validation | Before starting any task | Pass/fail checklist |
| `/debug-investigate` | Root cause analysis | When a bug is reported or test fails | Diagnosis with evidence |
| `/audit` | Codebase quality review | On demand or before major changes | Findings with severity |
| `/research` | Deep investigation of unfamiliar areas | Exploring new codebases or domains | Research summary with sources |
| `/code-review` | Structured review of changes | Before merging or after significant work | Findings with severity |
...
**Naming:** Use `/code-review`, not `/review`. The name `/review` conflicts with built-in agent commands (anti-pattern AP2, -3 deduction).
