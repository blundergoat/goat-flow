...
- After inspecting and trusting the checkout, use `goat-flow hooks verify . --agent <id> --scenario <deny-hook|post-turn-hook|gruff-hook> --trusted-target` to replay fixed offline inputs through one agent's exact configured command. Without the flag, the command returns unsupported evidence and does not start checkout hook code. A passing report proves only that local boundary; it does not prove the external provider fired the hook or showed the result to the model.
