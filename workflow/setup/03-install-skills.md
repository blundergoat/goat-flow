...
4. **goat-plan/SKILL.md** - Milestone planner and manager. Routes to read-only, small file-write, or full file-write mode by scope and authorization: authorized hotfix/small features get one or two compact milestone files, Standard+ gets an `ISSUE.md` overview plus milestones.
