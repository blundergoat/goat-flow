...
3. **Harness-only instruction requirements:** Verify the instruction file includes path-agnostic workspace boundary guidance and the canonical `.goat-flow/skill-docs/` (meta) and `.goat-flow/skill-docs/playbooks/` (tools) READ rules / Router Table rows from Step 02. Check commit guidance only when the target contains `.git`: normally the preferred `docs/coding-standards/git-commit-message.md` exists and the instruction file references it under `## Commit Messages`. Rename a former-only `docs/coding-standards/git-commit.md` only after confirming the destination is absent and every former-path reference is absent or confined to the selected agent's bridge. If another instruction file or section still references the former path, require the `skipped-references` result, preserve the former guide, and verify every reported reference still resolves. Preserve pre-existing dual guides while the bridge points to the preferred path. When `.git` is absent, verify that setup created neither guide and did not add a commit-guide bridge.
