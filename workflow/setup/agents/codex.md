...
- goat-flow registers the default Bash-focused `PreToolUse` deny hook, opt-in `PostToolUse` Gruff analysis, and default `Stop` safety for Codex. Treat registration and provider delivery separately: current `PostToolUse` and `Stop` delivery evidence is stale after the `commandWindows` registration change. Preserve registry-managed rows, and do not describe them as verified until event-specific capture renews the gate.
