...
- goat-flow currently registers only the Bash-focused `PreToolUse` deny hook for Codex. Do not add goat-flow `PostToolUse` or `Stop` entries to `.codex/hooks.json` unless a later ADR accepts a verified Codex lifecycle contract for that hook.
