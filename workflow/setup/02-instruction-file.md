...
Commit guidance prefers `docs/coding-standards/git-commit-message.md` - one file for humans and agents - and the instruction file carries a `## Commit Messages` summary that references it, because IDEs auto-read the instruction file, not the doc. If an existing project has only `docs/coding-standards/git-commit.md`, preserve and reference that accepted path; never create a second guide alongside it. Do not copy goat-flow's own commit policy into consumer projects. When neither accepted path exists, use goat-flow's git-history detector: sample `git -C <target> log -n 100 --pretty=%s`, classify subjects as conventional commits, ticket-prefixed, or free-form, and only name a dominant style when one class is at least 70% of the sample. If the installer already generated the preferred file, review and keep it. If history is unavailable or has fewer than 10 commits, keep the generated stub header (`goat-flow: generated stub - insufficient git history`) and note that the project owner should edit it later. The harness `commit-guidance` check accepts either docs path and reports `git-commit-message.md` as preferred.
