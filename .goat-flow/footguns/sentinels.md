...
- Hook stderr emission from a session-state hook (per `.goat-flow/tasks/related-improvement-ideas/M01-session-state-hooks.md`) — when the hook emits `<budget-pressure>...</budget-pressure>` and the agent reads it in its next observation, the block markers' position relative to other hook output matters.
