...
- `src/dashboard/dashboard-terminal.ts` (search: `TERMINAL_PASTE_COMMIT_DELAY_MS`) submits dashboard-launched pasted prompts as a second PTY input after the bracketed paste, so Claude Code can commit the pasted-text block before Enter is sent.
...
- `src/dashboard/dashboard-terminal.ts` (search: `spinner-glyph frame`) classifies a chunk whose entire stripped-plain content is just a spinner glyph as a transient redraw, so the runner's ~2 Hz idle-spinner frame stops killing the 1200ms reveal timer. Each runner uses a different glyph: Claude paints `●` (U+25CF), Codex paints `◦` (U+25E6 WHITE BULLET — distinct from `•` U+2022 and `·` U+00B7), other CLIs use braille patterns (U+2800–U+28FF). Live console instrumentation on 2026-05-21 traced 419 calls to `dashboardNextAwaitingInputState` over 60s with a Claude prompt visible: 208 true / 211 false alternating ~1/s, and the `awaitingInput` field never flipped to true even momentarily. After the Claude fix Codex still flickered set→clear within ~100ms on a 2.4s cadence until the glyph class was extended to include `◦` and the braille range.
- `test/unit/dashboard-terminal-launch.test.ts` (search: `keeps awaiting state across Claude Code's lone-bullet spinner frame`) and (search: `keeps awaiting state across Codex's lone-bullet spinner frame`) pin both runners' fixes with 8-tick simulations, plus assertions that real `● Now let me read…` and `◦ Running shell command:…` status lines still correctly clear the state (no false-stuck).
