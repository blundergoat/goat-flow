...
- **`CheckResult` / `HarnessCheckResult` schema evolution** (`src/cli/audit/types.ts`): a new field that changes semantics for some checks (e.g. `suggestedRevision` per `.goat-flow/tasks/related-improvement-ideas/M05-audit-suggested-revision.md`) can ride a sandwich layer rather than requiring all checks and consumers to update at once.
