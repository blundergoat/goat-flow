...
- PR #794 (merged 2026-03-24, same day as the disclosure, `michalcichon`). Supply-chain attack on litellm 1.82.7/1.82.8: injected a malicious `.pth` file on install, exfiltrated `OPENAI_API_KEY` / `AWS_SECRET_ACCESS_KEY`, attempted to establish a persistent backdoor in `~/.config/`, and could trigger a fork-bomb DoS.
...
- If the compromised dependency is a transitive (not direct) dep, either (a) pin the transitive via `overrides` in `package.json`, or (b) pin or update the intermediate direct dep that pulls it in. Document which approach was used so future reviews don't unwind the pin accidentally.
