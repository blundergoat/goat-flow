...
**Evidence (ACTUAL_MEASURED, 2026-08-10):** GNU Parallel's `--halt` was first treated as a standalone flag although the manual defines a required value, and a Bash short-option glob shadowed `--rcfile`; ShellCheck exposed the overlap, and direct probes showed the first correction still missed `-c` and `-cl`. Anchors: `workflow/hooks/deny-dangerous/guard-runtime.sh` (search: `--halt`), `workflow/hooks/deny-dangerous/patterns-shell.sh` (search: `A short option bundle containing`), `workflow/hooks/deny-dangerous/deny-dangerous-self-test.sh` (search: `parallel halt value before`).
