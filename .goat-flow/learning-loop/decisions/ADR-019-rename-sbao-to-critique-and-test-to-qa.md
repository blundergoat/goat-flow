...
**Updated:** 2026-08-04 - refreshed skill, CLI, and drift anchors after later contract refactors.
...
- `goat-test` collided with the ordinary developer meaning of "test". The skill body explicitly says it neither writes nor runs tests (`.claude/skills/goat-qa/SKILL.md` (search: `It neither writes nor runs tests`)) and repeats that prohibition in its boundary commands and constraints (`.claude/skills/goat-qa/SKILL.md` (search: `## Boundary Commands`) and `.claude/skills/goat-qa/SKILL.md` (search: `MUST NOT generate test code`)). Its actual scope spans testing-gap analysis, audit mode, regression guards, and targeted testing-plan output (`.claude/skills/goat-qa/SKILL.md` (search: `## Regression Guard Mode`) and `.claude/skills/goat-qa/SKILL.md` (search: `## Phase 3 - Targeted Testing Plan`)), which is broader than coverage and narrower than test execution.
- The earlier CLI rename from `critique` to `quality` removed the command-line namespace collision that would have made `/goat-critique` awkward. Commit `054bde2` (`2026-04-18`, `refactor(cli): rename \`critique\` command to \`quality\``) left `quality` as the canonical CLI subcommand (`src/cli/cli.ts` (search: `Agent-driven quality prompt plus save/history/diff surfaces`)). That earlier rename was about making the CLI self-describing; the side effect is that `critique` is now free for a skill name.
...
2. **Rename `goat-test` to `goat-qa`.** "QA" is the only shortlisted term broad enough to cover the skill's full scope without promising execution: testing-gap analysis, audit, regression guard, and targeted testing-plan output (`.claude/skills/goat-qa/SKILL.md` (search: `## Regression Guard Mode`) and `.claude/skills/goat-qa/SKILL.md` (search: `## Phase 3 - Targeted Testing Plan`)). The losing criterion was maximal familiarity: status-quo `test` is familiar, but it contradicts the skill's explicit no-execution contract (`.claude/skills/goat-qa/SKILL.md` (search: `It neither writes nor runs tests`) and `.claude/skills/goat-qa/SKILL.md` (search: `MUST NOT generate test code`)).
...
- **Positive:** `/goat-qa` stops over-claiming test execution. The command name no longer contradicts the skill's first-read contract and boundary commands (`.claude/skills/goat-qa/SKILL.md` (search: `It neither writes nor runs tests`) and `.claude/skills/goat-qa/SKILL.md` (search: `## Boundary Commands`)).
...
- **Prior CLI rename:** commit `054bde2` (`2026-04-18`) renamed the CLI subcommand `critique` to `quality`, freeing `critique` for skill use without a parallel CLI collision (`src/cli/cli.ts` (search: `Agent-driven quality prompt plus save/history/diff surfaces`)).
...
- `.claude/skills/goat-critique/SKILL.md` (search: `goat-critique runs only full delegated mode`)
- `.claude/skills/goat-qa/SKILL.md` (search: `It neither writes nor runs tests`)
...
- `src/cli/constants.ts` (search: `export function getSkillNames()`)
- `src/cli/audit/check-drift.ts` (search: `deprecated skill still installed`)
