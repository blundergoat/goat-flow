...
- **Correction, 2026-08-07 - the dashboard is NOT on `"static"`.** This entry previously claimed `dashboard-audit-routes.ts` "already audits selected targets at `static`". It does not, and did not when this entry was written. `src/cli/server/dashboard-audit-routes.ts` (search: `function buildDashboardAuditReport`) sets `agentFilter === null ? "present-only" : "full"`, so an explicit per-agent dashboard audit **executes the target launcher**. The `"static"` pair in that file belongs to `composeDashboardSetupOutput` - the *setup* path, not the audit path. The original author conflated the two. `git log -L` dates the `"full"` line to `927951bf` (2026-05-31), two weeks before this entry was created 2026-06-14, so the claim was wrong on day one and no regression occurred.
- **Reach, measured 2026-08-07.** Only a *selected-agent* audit executes. `src/cli/audit/audit.ts` (search: `function isAggregateAgentSkip`) skips agent-scope checks when no `--agent` is given, and `check-agent-deny-mechanism.ts` (search: `scope: "agent"`) declares no aggregate support. Measured on this repo: `audit .` reports `Agent deny mechanism: skipped`; `audit . --agent claude` reports `pass`. Do not restate this trap as "any `goat-flow audit` executes target code" - it needs `--agent`, or the dashboard per-agent path above.
...
**Prevention:** Keep an execution opt-out reachable. Do **not** assume the dashboard is already safe - per the correction above it requests `"full"` for per-agent audits and needs the same fix as the CLI. Never make runtime smoke the unconditional default for a surface that can audit untrusted checkouts. Before citing any surface as "already static", re-read the call site: this entry asserted that for 8 weeks without it being true, and the milestone written to fix the trap inherited the error. Treat any change to the default evidence level as a security decision - and note it can also flip a CI audit gate, because runtime smoke catches launcher / `$root` failures that static checks do not. Do not "harden" this by parsing the launcher and running only the managed script: that reintroduces the stale-path / broken-glue blind spot the full-command smoke exists to catch (see [hooks.md](hooks.md) search: `Hook command strings can fail before guard code starts`).
