...
**Recurrence 2026-09-21:** Extending the native Windows audit run found that `checkHookSelfTest` still selected bare `bash` after syntax checks had selected Git Bash. The focused dispatcher regression observed `bash` twice where native Git Bash was required. `src/cli/audit/check-agent-deny-mechanism.ts` (search: `runHookBash`) now shares shell selection, child PATH setup, and Windows argument normalization between both checks. The real self-test then reported `git: command not found`: `src/cli/install-invocation.ts` (search: `installerSpawnEnv`) read only uppercase `PATH` from a copied Windows environment whose actual key was `Path`, then created a competing truncated entry. Preserve the existing key and its inherited directories. Regression anchors: `test/unit/audit-command/agent-deny-hooks.test.ts` (search: `runs self-test with the selected agent dispatcher`) and `test/unit/install-invocation.test.ts` (search: `preserves Windows Path casing`). The later Windows drift suite exposed the same bare-Bash assumption in `src/cli/audit/check-agent-deny-runtime.ts` (search: `verifyDirectHookRuntime`); direct fallback now selects native Bash, passes the script as an argv operand, and supplies the provider payload on stdin. `test/unit/audit-command/agent-deny-hooks-drift.test.ts` (search: `ignores unrelated hooks whose names merely contain a managed script name`) retains the fallback regression. Registration-only cases call `checkHookRuntimeSmoke` directly so repeated full policy self-tests cannot time out before the behavior under test.
---
