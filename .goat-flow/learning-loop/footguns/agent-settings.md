...
**Why it happens:** Settings deny globs match as substrings across the whole command string, quoted arguments included. `.claude/settings.json` and the template `workflow/hooks/agent-config/claude.json` (search: `Bash(*git push*)`, search: `Bash(*git commit*)`) deny any command whose text mentions a guarded phrase. Since 2026-09-02 these are the only settings-layer Bash denies, because the `sudo`, `mkfs`, `dd`, and `git reset --hard` globs were retired under ADR-065 in favour of the hook's tokenizing parser. The hook layer does not share the trap: passing the same text as a `tool_input.command` payload to `.goat-flow/hooks/deny-git-mutations.sh` exits 0 for both denied shapes. That entrypoint supplies the setup required by `.goat-flow/hooks/deny-dangerous/guard-runtime.sh` (search: `tool_input.command`), which is a sourced helper. The self-test keeps read-only allow canaries at `workflow/hooks/deny-dangerous/deny-dangerous-self-test.sh` (search: `expect_allow`). The bluntness is deliberate per `.goat-flow/learning-loop/decisions/ADR-025-block-all-git-push.md`.
