...
5. Parse classification tags only from valid finding prefixes, give each active ID one provenance class, distinguish absent optional maps from explicit null, and match whole disclosed paths.
6. Re-run both the shared masker tests and the consuming proof-gate tests: `test/unit/plans-export.test.ts` (search: `masks type-7 custom-tag blocks without hiding later visible structure`) and `test/unit/review-validate.test.ts` (search: `rejects structural review evidence inside a type-7 HTML block`).
...
**Trigger phase:** SCOPE
**Caught at:** VERIFY
**Incident count:** 11 | **Latest occurrence:** 2026-09-14

**Prevention:** Treat a reforecast as one atomic edit to basis, range, headline, split, and every `(est: ...)` entry, then run strict validation before starting the timing receipt. A clean strict result is not consistency: recompute the `ISSUE.md` At a glance row, task band and Forecast summary rows from the milestone headers, because no checker validates them. After appending receipt time, reconcile measured-plus-remaining overview totals too; preserve the issued estimate instead of replacing it with Actual. When history runs under one minute per unit, the checker floors the advised likely at 1.00 min/unit and says so; copy those rates. Preserve planning-time estimates only when the advice still cannot be allocated: a `reforecast required` line whose likely rate is under 1.00 with no floor note, or `forecast advice` that says `issued values retained`. Derive bounds with floor, nearest, and ceiling, and never rescale an in-progress or completed milestone to newer calibration data.
...
**Why it happens:** `src/cli/plans-check-summary.ts` (search: `renderRequiredReforecasts`) advises the new rates, but `src/cli/plans-effort.ts` (search: `forecast basis derives`) requires the range to derive from the basis, `src/cli/plans-check.ts` (search: `must equal the Effort estimate total`) requires the headline to equal the range's likely value, and the headline is the sum of integer-only `(est: n min category)` entries in `src/cli/plans-effort.ts` (search: `TASK_ESTIMATE_PATTERN`). With a likely rate under 1 min/unit, N positive integer items cannot sum below N minutes, so the strict shape is unsatisfiable. Since 2026-09-20 the advice floors that likely: `src/cli/plans-effort.ts` (search: `function floorUnallocatableLikely`). `.claude/skills/goat-plan/references/milestone-examples.md` (search: `Reforecast all estimates`) already says "all".

**Incident ledger:**
- **Recurrence 2026-09-14 (forecast-accuracy M02):** the latest receipt recorded 4198 seconds, displayed as 70 minutes, while ISSUE and EXECUTION still used the earlier 3503 seconds / 58 minutes and a 308-minute measured-plus-remaining total. Strict validation exited 0 because `src/cli/plans-check.ts` (search: `loadPlanExportRecords(options.projectPath)`) reads milestone records, not the overview. The unchanged 210-minute remaining allocation yielded 320 displayed minutes before closeout work; receipt extensions require overview reconciliation even when no forecast changes.
...
- **Recurrence 2026-09-13 (forecast-accuracy M05/M06):** expanding repeated skill trials and adding default activation changed the counted work to 35 and 14 units, but a header replacement without multiline matching left the bases at 18 and 13. Strict validation caught both, including `forecast basis declares 18 agent work units but the plan contains 35`; the independent overview check also rejected the stale unit total. Correcting both bases restored 103 units and 277 likely minutes. `src/cli/plans-effort.ts` (search: `forecast basis declares`) owns the reproduced diagnostic. After scripted reforecasts, assert that every intended field was replaced and derive the overview from parsed saved headers before delivery.
