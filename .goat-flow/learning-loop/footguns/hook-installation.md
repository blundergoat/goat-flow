...
- **Recurrence 2026-08-06:** adding `run-with-bash.mjs` to every spec made per-spec matching treat the shared file as proof that any hook was installed, so drift tests removed or cross-matched unrelated entries. The shared recognizer excludes it: `src/cli/server/agent-hook-command.ts` (search: `commandsReferenceScriptToken(commands, spec.primaryScript)`).
...
7. The same order means a `cd` into another complete managed checkout, such as a clone under review, a sibling project, or an older or forked install, runs that checkout's policy and Stop launchers before `CLAUDE_PROJECT_DIR` is read; Claude's Gruff handler checks the session project first under ADR-066. ADR-052 treats that checkout as trusted executable content; return to the session project before relying on its policy.
...
## Footgun: Shared hook repairs can leave sibling status stale

**Status:** active | **Created:** 2026-09-05 | **Evidence:** ACTUAL_MEASURED
**Decision changed:** Reload every affected hook state after shared-file repair and invalidate retained proof against the repaired runtime.
**Trigger phase:** SCOPE
**Caught at:** VERIFY

**Prevention:** Separate toggle and registration ownership from shared-file effects. A successful single-hook response cannot describe every sibling whose installation or proof changed. Reload the complete hook list for the selected project, ignore responses after a project switch, and bind proof and audit caches to every required entrypoint and shared file.
...
**Prevention:** Preserve existing child indentation and unrelated bytes in both CLI and standalone migrations. Check parsed sibling settings as well as the newly added setting, then repeat migration to prove it converges. Flow mappings and whole-map aliases retain their separate handling.

**Symptoms:** Inserting a two-space Git entry before four-space hook siblings moved the existing dangerous and Gruff settings beneath the new entry. The installer then missed an enabled Gruff hook and failed to add its conventional binary override.
