...
**Trigger phase:** SCOPE
**Caught at:** VERIFY
**Incident count:** 12 | **Latest occurrence:** 2026-09-25
...
7. Enumerate command-bearing keys from the local `git-config` and `git-submodule` manuals when changing Git config inspection. Apply Git's `!` rules for custom submodule updates and credential-helper prefix rules before scanning values. Pair destructive probes with harmless values; check temporary `-c` values and saved `git config` writes.
