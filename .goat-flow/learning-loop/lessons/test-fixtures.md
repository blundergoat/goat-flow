...
**Status:** active | **Created:** 2026-04-03 | **Evidence:** OBSERVED
...
**Trigger phase:** ACT
**Caught at:** VERIFY
**Incident count:** 15 | **Latest occurrence:** 2026-09-06
...
- **Recurrence 2026-09-06 (review byte authority):** The initial area control used changed-line units instead of clusters, and a confident gate control retained a PARTIAL verdict. Re-reading `src/cli/review-validate-integrity.ts` (search: `validateFullSizeUnit`) and `src/cli/review-validate-verdict.ts` (search: `expectedShipVerdict`) isolated the authority failures. The new real-state suite also exposed Git's trailing-slash nested-repository marker and a `tsx` loader resolved from the temporary target cwd; `src/cli/review-validate-anchors.ts` (search: `untrackedPaths`) and `test/integration/review-validate-authority.test.ts` (search: `captures stdin and file requests`) retain the corrected boundaries. Whole-file lint and Gruff then exposed complexity and file-size limits. Splitting source capture, JSON, raw reads, and gate validation exposed a missing serializer import and inconsistent full/compact receipt whitespace. Typecheck and focused tests caught both; `src/cli/review-validate.ts` (search: `readGateReceiptField`) preserves full-field trimming and canonical compact JSON. Whole-suite proof then caught a dropped shared gate-causality rule and an unused legacy export; broader checks also caught stale help/evidence anchors and warning debt from unchecked type assertions. Read-only `test/contract/skill-hardening-shared-1.test.ts` (search: `classifies gate evidence without inventing changed-code causality`) and `src/cli/review-validate-authority.ts` (search: `function checkSnapshot`) retain the restored rule and typed revalidation boundary.

- **Recurrence 2026-09-06 (M23 follow-up):** A shared assertion context selected the wrong test block, and moving a regex through string replacement corrupted its `- **Recurrence 2026-09-06 (M22 follow-up):**` escape.
  Read-back caught both. Whole-file Gruff then caught oversized source and test files; re-read complete owners before narrowing helpers or placing regressions.
  Compressing overlap guidance also dropped the explicit non-suppression sentence; the existing four-way provenance contract caught it before handoff.
  Inlining the compact label check met the file limit but breached ESLint complexity; narrow the absent-scope branch before counting further relationships.
  Keep quoted and ordinary path controls together: `src/cli/review-validate-common.ts` (search: `disclosureNamesPath`) and
  `test/unit/review-validate.test.ts` (search: `retains all five selected files`). Replacement callbacks preserve literal `- **Recurrence 2026-09-06 (M22 follow-up):**` text in generated edits.

- **Recurrence 2026-09-06 (M22 follow-up):** The first review suite missed valid anchors containing `anchor=`, overridden Git settings, rounded fractions,
  and an unsupported checkout changing an immutable review identity. Five targeted cases failed before repair and passed afterward.
  `test/integration/review-validate-authority.test.ts` (search: `keeps anchor markers`; `effective autocrlf`; `unrelated checkout becomes unsupported`)
  retains the real Git/report controls, including a project directory ending in whitespace.
  `src/cli/review-validate-common.ts` (search: `isExactIntegerLiteral`) checks the number's decimal spelling before rounding can change a parent selection.
  Five measured index reads at 20,000 entries exposed repeated suffix decoding: median 13054.90 ms before and 36.99 ms after the bounded read.
  `src/cli/review-validate-anchors.ts` (search: `nextPathEnd`) retains the linear read boundary; these timings describe that reader, not whole-report latency.

- **Recurrence 2026-09-06 (review integrity):** Receipts written after source capture changed the workspace and hid the intended failures. Arrange ignored logs and selected files before capture; `test/unit/review-validate.helpers.ts` (search: `reviewAuthorityFields`; `createReviewedProject`) owns that order.
  - Parser and fixture refactors exposed wrong scalar-reader arguments, missing imports, and confusion between PR targets and `source.requested.sample`. Check both producer and consumer shapes: `src/cli/review-validate.ts` (search: `textField(gate.id`), `src/cli/review-validate-sections.ts` (search: `area request`), and `test/integration/review-validate-authority.test.ts` (search: `rejects a receipt replaced`).
  - A filename containing `anchor=` reached the generic anchor scanner; `test/integration/review-validate-authority.test.ts` (search: `keeps anchor markers`) now protects that boundary. A stale provenance replacement silently did nothing; `test/unit/review-validate.helpers.ts` (search: `withIntegrityFields`) rejects ambiguous field mutations.
