...
- **Recurrence 2026-08-01 (PR #57 hardening):** An under-specified patch context shared by the adjacent estimate and Actual parsers put `!match` where only `estimateMatch` exists; re-reading both complete functions fixed it. `src/cli/plans-effort.ts` (search: `estimateText.match(EFFORT_ESTIMATE_PATTERN)`).
- **Recurrence 2026-08-01 (PR #57 preflight):** Focused tests, typecheck and the package suite were green before preflight reported seven complexity failures in the hardened plan and review parsers; splitting lifecycle, numeric-field, fence-state and ledger checks into named helpers preserved the contract, and `stats --check` caught the anchor that refactor removed from this entry. `src/cli/plans-check.ts` (search: `collectNotStartedSnapshotErrors`), `src/cli/plans-effort.ts` (search: `readEffortNumbers`), `src/cli/rendered-markdown.ts` (search: `isFencedLine`), `src/cli/review-validate-ledger.ts` (search: `readReviewReceipt`).
- **Recurrence 2026-08-02:** Removing unconsumed M01 exports made Knip green but left the now-internal Actual-state alias unreferenced; typecheck and ESLint failed until it was deleted. `src/cli/plans-effort.ts` (search: `export interface PlanEffortNumericActual`).
...
- **Recurrence 2026-08-16 (review anchors):** Replacing `<target-project>/path` with a literal sample path made the anchor sweep treat it as live evidence, and `<repo-relative-path>` still failed because the verifier exempted one historical token rather than the placeholder grammar. `test/contract/skill-hardening.helpers.ts` (search: `angle-bracket token`), `test/contract/skill-hardening-review-2.test.ts` (search: `documents validator-ready anchors`).
- **Recurrence 2026-08-16 (verification authority):** Formatting two fixtures found by the changed-file gate while a full fast-suite run was still active changed that run's authority, so its result was discarded. `package.json` (search: `prettier --write`), `test/integration/gruff-code-quality-contract.test.ts` (search: `surfaces a symbol finding when its span overlaps`), `test/unit/review-validate-verdict.test.ts` (search: `accepts eight REFUTED ledger records`).
...
- **Recurrence 2026-09-06 (review byte authority):** The initial area control used changed-line units instead of clusters and a confident gate control kept a PARTIAL verdict; re-reading `src/cli/review-validate-integrity.ts` (search: `validateFullSizeUnit`) and `src/cli/review-validate-verdict.ts` (search: `expectedShipVerdict`) isolated both. The new real-state suite exposed Git's trailing-slash nested-repository marker and a `tsx` loader resolved from the temporary target cwd; `src/cli/review-validate-anchors.ts` (search: `untrackedPaths`) and `test/integration/review-validate-authority.test.ts` (search: `captures stdin and file requests`) retain the corrected boundaries. Splitting source capture, JSON, raw reads and gate validation for the lint and Gruff limits exposed a missing serializer import and inconsistent receipt whitespace, both caught by typecheck and focused tests; `src/cli/review-validate.ts` (search: `readGateReceiptField`) preserves full-field trimming and canonical compact JSON. Whole-suite proof then caught a dropped shared gate-causality rule, an unused legacy export, stale help/evidence anchors and unchecked type assertions; `test/contract/skill-hardening-shared-1.test.ts` (search: `classifies gate evidence without inventing changed-code causality`) and `src/cli/review-validate-authority.ts` (search: `function checkSnapshot`) retain the restored rule and typed revalidation boundary.
- **Recurrence 2026-09-06 (M23 follow-up):** A shared assertion context selected the wrong test block, and moving a regex through string replacement corrupted its `- **Recurrence 2026-09-06 (M22 follow-up):**` escape; read-back caught both. Whole-file Gruff then caught oversized source and test files, compressing overlap guidance dropped the explicit non-suppression sentence until the four-way provenance contract caught it, and inlining the compact label check met the file limit but breached ESLint complexity. Keep quoted and ordinary path controls together: `src/cli/review-validate-common.ts` (search: `disclosureNamesPath`) and `test/unit/review-validate.test.ts` (search: `retains all five selected files`); replacement callbacks must preserve literal `- **Recurrence 2026-09-06 (M22 follow-up):**` text in generated edits.
- **Recurrence 2026-09-06 (M22 follow-up):** The first review suite missed valid anchors containing `anchor=`, overridden Git settings, rounded fractions and an unsupported checkout changing an immutable review identity; five targeted cases failed before repair and passed afterward. `test/integration/review-validate-authority.test.ts` (search: `keeps anchor markers`; `effective autocrlf`; `unrelated checkout becomes unsupported`) retains the real Git/report controls, including a project directory ending in whitespace, and `src/cli/review-validate-common.ts` (search: `isExactIntegerLiteral`) checks the number's decimal spelling before rounding can change a parent selection. Five measured index reads at 20,000 entries exposed repeated suffix decoding, median 13054.90 ms before and 36.99 ms after the bounded read; `src/cli/review-validate-anchors.ts` (search: `nextPathEnd`) retains the linear read boundary, and those timings describe that reader, not whole-report latency.
- **Recurrence 2026-09-06 (review integrity):** Receipts written after source capture changed the workspace and hid the intended failures; `test/unit/review-validate.helpers.ts` (search: `reviewAuthorityFields`; `createReviewedProject`) owns the arrange-before-capture order. Parser and fixture refactors exposed wrong scalar-reader arguments, missing imports and confusion between PR targets and `source.requested.sample`; check both producer and consumer shapes in `src/cli/review-validate.ts` (search: `textField(gate.id`), `src/cli/review-validate-sections.ts` (search: `area request`) and `test/integration/review-validate-authority.test.ts` (search: `rejects a receipt replaced`). A filename containing `anchor=` reached the generic anchor scanner, now protected by `test/integration/review-validate-authority.test.ts` (search: `keeps anchor markers`), and a stale provenance replacement silently did nothing, now rejected by `test/unit/review-validate.helpers.ts` (search: `withIntegrityFields`). Focused tests missed two shared guidance failures, unused exports and stale moved-test citations; run the shared contracts and full preflight before freezing the handoff, per `test/contract/skill-hardening-shared-1.test.ts` (search: `teaches compact clean output`; `keeps public skill workflows aligned`).
- **Recurrence 2026-09-07 (release-plan reconciliation):** A revised ISSUE checker rejected a valid completed row because its forecast regex excluded historical actual-time annotations; comparing every likely field then exposed unfinished rows with old or missing values. Read every producer shape and reconcile all repeated fields from `src/cli/plans-export.ts` (search: `loadPlanExportRecords`); the range owner `src/cli/plans-effort.ts` (search: `PlanEffortForecastRange`) binds likely to the milestone total. Run the documented command against the actual document, then mutate totals, dependencies and checkboxes in memory to test rejection. Committed evidence must cite tracked owners; `src/cli/facts/shared/learning-loop-common.ts` (search: `gitignored path used as durable evidence anchor`) rejected the first recurrence's local-plan citations.
- **Recurrence 2026-09-13 (installer preservation):** A hand-written legacy baseline used compact JSON without the producer's trailing newline, so preview rejected the fixture before the preservation regression ran. Match the canonical bytes from `src/cli/managed-setup-state.ts` (search: `writeManagedInstallState`); `test/integration/setup-install-migrations.test.ts` (search: `preserves retired writing playbooks`) now reaches preview, installation and exact-content assertions.
- **Recurrence 2026-09-13 (plan estimates):** A first draft copied the fractional per-unit rate into task estimates and the strict checker rejected it. Rates may be fractional, but task minutes must match `src/cli/plans-effort.ts` (search: `TASK_ESTIMATE_PATTERN`): allocate whole-minute tasks that preserve the derived total, category sums and forecast range, and validate before implementation.
...
**Recurrence 2026-09-16:** M10 first changed a stored hash without rebuilding its generation, then hand-serialized managed state. The strict reader refused both fixtures before policy consent could be tested. Use the production row builder and writer for a pristine prior install, and compare parsed YAML choices when formatting is not the contract. Evidence: `test/integration/hook-sync-recovery.test.ts` (search: `mixedPolicyProject`), `src/cli/managed-setup-state.ts` (search: `createManagedInstallStateRow`).

**Recurrence 2026-09-17:** Packaged 1.15.0 and 1.16.0 upgrade fixtures supplied old launcher bytes without install history, then expected automatic Sync.
The changed launcher correctly required replacement review. Keep that refusal and preservation proof, then exercise exact consent before expecting Sync.
Evidence: `test/integration/packaged-hook-install.test.ts` (search: `V1_16_0_BASH_RUNNER`) and
`src/cli/server/hook-operation.ts` (search: `reviewMessage`). The corrected cases verify refusal, exact replacement consent and archived CLI convergence.


**Recurrence 2026-09-17 (archived recovery):** M12 first imported a spawn helper from its consumer, then assumed the archived package had current hook files, then treated registration order after a toggle as fixed.
Read the exported owner and pinned manifest before writing an archived fixture; compare the contract the operation preserves and require a second Sync to converge exactly.
The corrected fixture captures the old general-policy handler before upgrade and the new Git handler afterward; it preserves all commands, user rows and unrelated files.
Evidence: `test/integration/hook-sync-recovery.test.ts` (search: `installArchivedCodexProject`, `savedCodexPolicyCommand`, `syncedConfig`) and `src/cli/server/agent-hook-command.ts` (search: `agentHookSpawnDescriptor`).

The state-only follow-up repeated the fixture mistake with a noncanonical legacy receipt and an assumed clean full install after Sync.
Use the reader's canonical receipt bytes; preserve the separate unmanaged-file refusal, then approve only the named replacement before checking convergence.
