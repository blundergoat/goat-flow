...
**Recurrence 2026-09-05:** The independent Git policy's own corpora passed while adjacent fixtures still omitted the shared helper, selected the first registration, or expected diagnostics without the affected hook's name. The standalone-preview fixture also expected unchanged YAML despite the required one-time inherited Git choice. Re-read each assertion against its owning contract: complete the fixture store, select the exact policy entrypoint and retain precise expected outcomes. `test/integration/hook-command-spawn-matrix.test.ts` (search: `registeredHandler`), `test/integration/setup-install-preview.test.ts` (search: `limits force to managed conflicts`), and `test/unit/hook-launcher.test.ts` (search: `deny-git-mutations`) cover the repaired boundaries. Keep feedback-hook failure identity separate from policy identity; a broad launcher override had also changed two Gruff outcomes before its scope was narrowed.

**Recurrence 2026-09-19:** Release verification found 19 slow-suite failures left by the managed policy-state contract: fixtures without the new policy reader, disabled-hook assertions expecting removed registrations, a row picked by position, and refusals and reviews that now arrive earlier. Preflight and the fast suite had passed because neither runs those files. `test/integration/setup-install-migrations.test.ts` (search: `hook-policy-state.cjs`), `test/integration/hook-effective-state.test.ts` (search: `function denyDangerousEntry`).

**Further verification (2026-09-05):** Whole-tree checks also exposed stale version/scenario fixtures, generated bootstrap copies, five complexity findings and a four-space YAML migration failure. Run ESLint and TypeScript typecheck before citing transpiled test results; the first classifier extraction and its follow-up punctuation fix failed, so that extraction was rewound before using explicit registration predicates. `src/cli/audit/harness/check-constraints.ts` (search: `hasRegisteredPolicyHooks`) and `src/cli/config/writer.ts` (search: `hookChildIndent`) preserve the corrected contracts. The existing installer test `test/integration/setup-install.test.ts` (search: `repairs a missing gruff-py override for an enabled existing hook`) reproduced the indentation bug; it and the focused writer test both passed after preserving the sibling indent. The next complete warning-ratchet run exposed an eighth positional parameter in the launcher after the exact generated-payload exception was repaired. Grouping the failure details preserved launcher outcomes and returned the ratchet to zero warning identities. `workflow/hooks/run-with-bash.mjs` (search: `reportLauncherUnavailable`) owns that boundary; run the complete gate again after clearing its first blocker. The harness audit then rejected a shortened helper path in the architecture table; cite the complete repository-relative file even when adjacent table columns supply context. `.goat-flow/architecture.md` (search: `workflow/hooks/deny-dangerous/guard-runtime.sh`) and `src/cli/audit/harness/check-context.ts` (search: `doc-paths-resolve`) own the repaired reference and its check.
...
**Prevention:** Run each persisted anchor exactly as a future agent will: single-quote the whole pattern, or use `-F` with plain tokens, and keep Markdown backticks, emphasis, and code-span punctuation out of shell arguments entirely. Use direct stdin for formatted prose, resolve every path and symbol against current source before persisting it, mark future paths task-owned, and reject unresolved or shell-diagnostic output. Evidence anchors: `workflow/hooks/deny-dangerous/guard-runtime.sh` (search: `Backtick command substitution hides nested execution`) blocks the unsafe shape; `src/cli/redact-command.ts` (search: `readFileSync(0`) accepts direct stdin; `.goat-flow/architecture.md` (search: `## Local Data and Evidence Budget`) is a formatting-independent anchor.
