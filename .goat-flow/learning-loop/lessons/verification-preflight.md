...
**2026-08-10:** A launcher helper rename left one learning-loop search anchor pointing at the removed function, so the repository schema gate failed after focused code checks cleared. Pair semantic renames with an exact old-name search across durable evidence before preflight. Evidence anchors: `src/cli/server/agent-hook-command.ts` (search: `legacyHookLaunchMode`) and `scripts/preflight-checks.sh` (search: `Learning-loop schema`).
