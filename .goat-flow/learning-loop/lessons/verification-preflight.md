...
**Decision changed:** Run ESLint, Knip, and formatting before preflight after TypeScript surface changes. | **Trigger phase:** VERIFY | **Incident count:** 12 | **Latest occurrence:** 2026-08-09

**What happened:** Focused tests and typecheck repeatedly cleared before later static gates. On 2026-08-09, formatting found one touched contract and full preflight found estimator complexity 15/11. Earlier recurrences included an undiscovered worker and impossible fallback.
...
**Measured recurrences:** Static gates have found lint complexity, impossible conditions, internal-only exports, an undiscovered worker, and a measured Knip heap limit after narrower checks cleared. On 2026-08-09, they found formatting drift and estimator complexity 15/11. Evidence anchors: `test/contract/skill-hardening-plan-1.test.ts` (search: `Never use duration intuition`) and `src/cli/plans-effort.ts` (search: `hasCompleteForecastBasis`).
