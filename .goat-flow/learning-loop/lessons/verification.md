...
**Recurrence 2026-09-04 (release prose):** The v1.17.0 release-prose verifier first failed on its own syntax, then failed again because it expected an abbreviated audit command instead of the full version-pinned command in the approved draft. Neither failure described an artifact defect. After two corrections, the approach was rewound to direct literal checks against the reread prose. The affected draft is gitignored, so it is deliberately not cited as a durable learning-loop anchor; the evidence was the two failed proof outputs followed by the same-session exact artifact read. The durable control is `.goat-flow/skill-docs/skill-preamble.md` (search: `Proof Gate`).

**Recurrence 2026-09-04 (release repair):** An M38 orchestration wrapper passed literal `\n` text between shell fragments, so Bash read `pipefailngit`, exited 127, and ran neither intended hash command. The exact retry used real separators, exited 0, and reproduced all accepted hashes. In the same repair episode, an exact-context patch mismatch remained atomic and was not claimed as a write; the first strict M78 check then exited 1 on two invalid plain-language fields, which were corrected from its literal diagnostics before activation. A later all-in-one clarity inventory was blocked before execution because the live guard rejected its compound shape, so the retry used bounded read-only commands. Tracing both scanner providers also corrected a grep-specific `REVIEW` exit comment to provider-neutral findings wording. After the human gate, a terminal check invoked the nonexistent singular `plan` command and exited 2; rereading the milestone's command ledger exposed the syntax error, and the exact `plans check ... --strict` command exited 0. The first post-repair release-prose wrapper then searched the changelog for wording that exists only in the derived release draft; its first three assertions passed, the mismatched fourth assertion stopped execution before any hash claim, and rereading both artifacts identified the changelog-owned wording. Its corrected successor then assumed `rg -c` prints numeric zero for no matches; ripgrep exited 1 with empty stdout, so the comparison failed before hashing. Two corrections on that wrapper approach triggered a rewind to independent prose, count, identity, and plan checks. The next M37 cut's first predecessor-residue wrapper searched ordinary diff context and exited 1 on an unchanged `1.16.0` changelog line next to the new audit-timeout bullet. Its zero-context, added-line-only retry printed `M37_NEW_PREDECESSOR_HITS: PASS (zero added lines)` and independently counted 63 matches in both committed HEAD and the worktree, proving a verifier false positive rather than a candidate defect. The release plans are gitignored, so their outputs remain session evidence; durable controls are `.goat-flow/skill-docs/skill-preamble.md` (search: `Proof Gate`), `CHANGELOG.md` (search: `Preflight bounds dependency audits`), `src/cli/help.ts` (search: `Advanced commands:`), `src/cli/plans-check-structure.ts` (search: `plain-language section`), and `scripts/maintenance/scan-secrets.sh` (search: `Using gitleaks`).
...
**Prevention:** Treat a bucket split as a rename with unknown inbound callers. Before regenerating, grep the moved entry titles across committed content, not just the bucket directory; a scoped search that filters to the lessons tree misses ADRs, playbooks, and instruction files. After regenerating, run both gates: `stats --check` proves the buckets and generated indexes are internally consistent, and `goat-flow audit --check-content` proves outside documents still resolve their cited anchors. Editing any ADR body also stales `decisions/INDEX.md`, so regenerate after the citation repair rather than before it.

**What happened:** Splitting four entries out of `lessons/verification.md` into `lessons/verification-environment.md` moved the anchor `Parallel sessions need concurrency-safe file patterns`. The bucket README's own post-split instruction says to re-run `goat-flow index` and let `stats --check` find any anchor that pointed at a moved entry. Both ran clean. The break surfaced two steps later in preflight as a cold-path lint warning, because `ADR-048-concurrent-session-detection.md` cited the entry by its old bucket path. Repairing the ADR then staled `decisions/INDEX.md`, which failed the next preflight until the index was regenerated a second time.

**Evidence:** `.goat-flow/learning-loop/decisions/ADR-048-concurrent-session-detection.md` (search: `Parallel sessions need concurrency-safe file patterns`) - the inbound citation that broke; `src/cli/audit/check-content-quality.ts` (search: `stale-semantic-anchor`) - the rule that caught it; `.goat-flow/learning-loop/lessons/README.md` (search: `Bucket Size`) - the post-split instruction that named only `stats --check`.
