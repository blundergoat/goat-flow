...
**Recurrence 2026-08-27:** Running `npm run build` and the packaged-hook canary concurrently let the build delete and recreate `dist` while the test archived the package. The archive exposed the declared CLI path without the corresponding file, so the combined run failed 91/92 at `existsSync(packedCliTargetPath)`. The isolated canary after build completion passed 2/2, proving verification-command interference rather than a package regression. Evidence anchors: `package.json` (search: `rmSync('dist'`) and `test/integration/packaged-hook-install.test.ts` (search: `existsSync(packedCliTargetPath)`).
