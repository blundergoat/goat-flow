...
- **Recurrence 2026-09-18:** Removing validator non-null assertions added guard conditions that raised `indexEntries` to complexity 11 against the observed ESLint limit of 10. Three typechecks and 104 validator tests per batch passed before scoped lint caught it. Keeping the guarded details match and decoding its required hexadecimal capture with an explicit prefix cleared lint; typecheck and all 104 tests were rerun. Evidence: `src/cli/review-validate-anchors.ts` (search: `indexEntries`), `eslint.config.mjs` (search: `complexity`).
