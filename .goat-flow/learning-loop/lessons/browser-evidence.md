...
**Scope:** Proving browser-visible behaviour - live runner runs rather than timer units, serving local HTML over localhost, version-matched wrapper smokes, and reusing a route's real inputs instead of its composer. Dashboard-specific test mechanics are [dashboard-testing.md](dashboard-testing.md).
