...
**Status:** active | **Created:** 2026-06-05 | **Incident count:** 3 | **Latest occurrence:** 2026-08-29
...
**Recurrence 2026-08-29:** PR #61's configured-handler verifier reintroduced Node-owned `spawnSync` input on Windows. In the hosted `windows-hook-contracts` job under Node 20.11.0, three deny scenarios crossed the 30-second probe timeout while the read-only control completed just below it. A native Windows 11 control did not reproduce the stall. The first correction moved the fixed payload into an outer PowerShell pipe; the next two hosted runs still failed, with a timeout followed by two false allows. Both attempted transports left EOF delivery to a pipe spanning nested PowerShell, Node, and Bash processes, while the hook reads all stdin with `cat`. The third correction kept the registered executable and arguments but gave Windows shell handlers a finite read-only file as stdin; its first hosted run passed the exact two-file command. During Linux verification, npm also exported the temporary Node installation as `npm_config_prefix`; the test's login shell sourced the host's NVM setup and wrote unrelated stderr. Repeating the suite with an NVM-compatible prefix kept Node 20.11.0 active and passed, so preserve child stderr and rule out host startup files before attributing this class of failure to the hook. Evidence anchors: `src/cli/hooks-runtime-evidence.ts` (search: `spawnManagedConfiguredProbe`) and `test/unit/hooks-runtime-evidence.test.ts` (search: `selects file-backed Windows input`).

**Outcome 2026-08-29:** At PR head `c1ae99f`, the [hosted Windows job](https://github.com/blundergoat/goat-flow/actions/runs/33223270065/job/99021672169) reported `# tests 33`, `# pass 33`, `# fail 0`, and `# skipped 0`. A same-fixture diagnostic replayed one fixed blocked payload through four exact-handler variants: full environment plus pipe took 1,427 ms, managed environment plus pipe 31,305 ms, full environment plus file 1,397 ms, and managed environment plus file 25,023 ms; every row returned `status=2` with `error=none`. This one hosted sample showed that file-backed input was not the whole explanation: both managed-environment rows were much slower than their full-environment controls, while file input moved the managed replay from above the production 30-second deadline to below it. It does not identify a particular environment variable or establish a stable performance bound. For hosted-only latency, keep the handler, fixture, and payload fixed; vary environment and stdin independently; then repeat the distinguishing variant before treating one green rerun as stable. Evidence anchors: `test/integration/hook-command-spawn-matrix.test.ts` (search: `codex-windows-replay`), `src/cli/hooks-configured-runtime-evidence.ts` (search: `managedHookEnvironment`), and `src/cli/hooks-runtime-evidence.ts` (search: `spawnManagedConfiguredProbe`).
