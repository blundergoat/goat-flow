...
**Recurrence update (2026-08-10):** Fresh Codex canaries promoted Gruff and Stop to effective support, but the registry used the nonexistent final status `scenario-verified` and an integration test still selected Codex as its unsupported-provider example. Typecheck rejected the status, then the focused effective-state suite exposed the stale provider choice. The registry now uses `effective` after current proof, while the unsupported assertion names Antigravity and preserves its absent-capture outcome. Evidence anchors: `src/cli/server/hooks-registry.ts` (search: `effectiveSupportGate: "effective"`), `test/integration/hook-effective-state.test.ts` (search: `antigravityState`), and `test/integration/hook-effective-state.test.ts` (search: `expires approved Codex live support after its capture window`).
