...
├── diagnostics-command-parser.ts = lightweight `diagnostics context|bundle` positional rules
├── diagnostics-command.ts      = lazy context/support collection and format dispatch
├── diagnostics/                = read-only context-pressure and redacted support-bundle schemas/renderers
