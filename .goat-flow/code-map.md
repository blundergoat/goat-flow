...
├── diagnostics-command-parser.ts = `diagnostics context|readiness|bundle` positional rules
├── diagnostics-command.ts      = lazy context/readiness/support collection and format dispatch
├── diagnostics/                = read-only context, readiness, and redacted support schemas/renderers
