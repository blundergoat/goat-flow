...
Shared repairs refresh every affected dashboard row and invalidate audit caches and retained policy proof. Proof binds each hook and its complete scenario set to the current trusted runtime bytes and file revision; restoring old bytes does not revive earlier proof. The existing single-hook toggle response stays unchanged; the dashboard reloads the complete list. Configured-command replay does not establish external-provider delivery.
