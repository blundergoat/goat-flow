...
`deny-dangerous` owns destructive shell, secret-path and GitHub CLI protection. `deny-git-mutations` owns native Git commit, publication and destructive history/cleanup policy. Two thin, fixed-policy entrypoints share one parser and response implementation; payloads and environment variables cannot select their policy. Commit and push remain developer-only actions under ADR-025.

Both policies default on. A missing Git-hook choice inherits the legacy dangerous-hook choice once, before another toggle changes it; explicit choices survive. Registrations are owned by exact primary entrypoints, while each registry entry declares its full shared dependency set. Installation and inspection deduplicate that set by managed path. Enabled Git registrations are established before narrowing an enabled legacy policy, and interrupted partial stores fail closed.
...
| Forecast context and history | `src/cli/plans-forecast-context.ts`, `src/cli/plans-forecast-history.ts`, `src/cli/plans-forecast-model.ts` | Immutable issued snapshots and closed-cutoff remaining revisions; physically contained project history with per-plan `evaluation/prospective-registration.json` provenance; opt-in matching by known work state, scope and rubric. Sparse evidence retains the selected-plan numerical fallback; broad pooling is diagnostic, legacy exports stay unchanged without context, and default promotion requires prospective qualification |
