...
`deny-dangerous` owns destructive shell and secret-path protection. `deny-git-mutations` owns Git commit, publication, destructive history/cleanup and GitHub CLI write policy; its display name is **Deny Git and GitHub writes**. ADR-028 preserves GitHub reads and issue/PR comments. Two thin, fixed-policy entrypoints share one parser and response implementation; payloads and environment variables cannot select their policy. Commit and push remain developer-only actions under ADR-025.

Both policies default on at invocation. Explicit config migration during upgrade records a missing Git-hook choice from the legacy dangerous-hook choice once; explicit choices survive. Ordinary Sync and toggles preserve saved choices and registry defaults, including a missing/default-on Git choice. Registrations are owned by exact primary entrypoints, while each registry entry declares its full shared dependency set. Installation and inspection deduplicate that set by managed path. Enabled Git registrations are established before narrowing an enabled legacy policy, and interrupted partial stores fail closed.

Disabled policies retain their owned registrations. workflow/hooks/hook-policy-state.cjs and its installed mirror use the locked vendored js-yaml parser to reject invalid YAML, nonboolean policy choices and conflicting legacy aliases. Trusted reads resolve the selected physical root, reject linked/nonregular/shared config files, and compare one descriptor with its path before returning a decision. The launcher validates the requested script before reading config; only explicit false returns policy success before Bash discovery, without scan proof. Missing choices remain on, including a missing Git choice before migration. This is the trusted-checkout contract of ADR-052, not concurrent-attacker containment.

Disable refuses incompatible existing launch dependencies before saving; reviewed Sync can refresh those dependencies while preserving dormant policy-only bytes. Disabled current policies remain neutral with retained registration, while incompatible installations retain repair diagnostics. The standalone installer consumes generated disabled-retention metadata and ships both reader and parser without requiring target node_modules. Local saved-handler replay establishes fixture compatibility; provider reload and live-session recovery remain unverified.

Shared repairs refresh every affected dashboard row and invalidate audit caches and retained policy proof. Proof binds each hook and its complete scenario set to the current trusted runtime bytes and file revision; restoring old bytes does not revive earlier proof. A successful toggle preserves its `hook` response and adds the complete `hooks` list; collection sync returns all rows after verification. Configured-command replay does not establish external-provider delivery.

Hook sync uses the templates bundled with the running CLI and preserves persisted choices. `src/cli/server/hook-operation.ts` prepares the complete destination set, checks physical paths and canonical history, and holds ADR-048 claims through apply, verification, and ADR-064 hook-only publication. It preserves every full-install receipt. Differing unknown or diverged hook files need explicit confirmation tied to the exact review; unsafe/newer files and invalid history remain blocked. Partial applies report changed paths and recovery, and stale client responses cannot affect another project or Hooks visit.

Mixed-policy ownership upgrades require independent policy consent before write claims and again before apply under those claims. The portable decision compares original and requested choices plus original/incoming ownership bytes, including missing or unclassified files; equal choices and fresh installs preserve defaults without this review. The dashboard binds consent to the selected project, action, config, package bytes and complete destination set, even when file conflicts are empty. Replacement consent remains separate. Normal CLI and standalone installation refuse pending review before target writes, including force and CLI-admitted paths; CLI repeats the check before install-state preparation. Byte-identical ownership files do not prompt again.
...
| Hook scripts | `workflow/hooks/` (source), `.goat-flow/hooks/` (installed, active) | Managed launcher/runtime and provider adapters, the independent `deny-dangerous.sh` and `deny-git-mutations.sh` policy entrypoints with one shared `workflow/hooks/deny-dangerous/guard-runtime.sh` parser/response implementation, three policy modules and a routed self-test corpus, the dependency-free saved-choice reader and GitHub GraphQL query reader with their vendored parsers, opt-in `gruff-code-quality.sh`, default `post-turn-safety.sh`, and per-agent config templates. `workflow/hooks/` is the canonical source; its runtime scripts install in lockstep to `.goat-flow/hooks/` (this repo included), the copy the agent's shell actually runs against |
...
| Hook registration, contracts, and proof | `src/cli/hooks-command.ts`, `src/cli/hook-contracts.ts`, `src/cli/hooks-runtime-evidence.ts`, `src/cli/server/hooks-registry.ts`, `src/cli/server/hook-registrar.ts`, `src/cli/server/hook-operation.ts`, `src/cli/server/agent-hook-writer.ts` | Provider-neutral evidence/result contracts, CLI/dashboard hook toggles, and explicit bounded managed-hook classifier proof backed by manifest specs, installed-agent detection, and per-agent config state |
...
| Forecast context and history | `src/cli/plans-forecast-context.ts`, `src/cli/plans-forecast-history.ts`, `src/cli/plans-forecast-model.ts` | Immutable issued snapshots and closed-cutoff remaining revisions; physically contained project history with provenance from `prospective-registration.json` in each plan's `evaluation` directory; opt-in matching by known work state, scope and rubric. Sparse evidence retains the selected-plan numerical fallback; broad pooling is diagnostic, legacy exports stay unchanged without context, and default promotion requires prospective qualification |
...
| **Local coordination state** | `.goat-flow/state/locks/*.claim` | No (gitignored by design; no marker is committed) | Path-keyed exclusive ownership markers held by cooperating writers across one write transaction. Claims do not expire or permit stealing; remove an abandoned claim only through explicit operator-confirmed recovery after confirming no active owner. Brief `.recovery` guards beside claims serialize that removal and require inspection if abandoned. |
...
| **Local install evidence** | `.goat-flow/state/install/managed.json`, `.goat-flow/state/install/<agent>.json` cutover markers | No (gitignored by design; no baseline is committed) | The authoritative managed-path hash baseline a later `install`, `setup --dry-run`, or hook sync compares against, per ADR-064. Verified hook operations may advance only their dependency rows and preserve all full-install receipts. An absent baseline is the normal state for a checkout populated from Git rather than the installer, and `goat-flow status` reports it as `baselineStatus: missing` rather than as damage. |
...
Operational state shares the gitignored `.goat-flow/state/` parent. Public install relocates legacy `.goat-flow/install-state/` and `.goat-flow/write-claims/` to `state/install/` and `state/locks/` after admission and before acquiring the ordinary write claims. Directory relocation preserves record bytes and refuses outstanding claims, unsafe paths or occupied destinations. Operators must stop and upgrade every writer before migration; old paths are removed and older-version execution afterward is unsupported. Dry-run reads legacy install evidence without migration, and claims inspection/recovery can read the sole legacy claim namespace. Hook changes and `learn new` require public install while legacy storage remains. Claim coverage is unchanged.

The explicit `install --migrate-state-only` action resolves a legacy-state/policy-review deadlock without installing files or granting policy consent.
It validates the selected project and existing install evidence, then uses the same relocation primitive and its unsafe-path, claim and destination checks.
It preserves record bytes, registrations and policy choices, emits no completed-install receipt, and makes no changes when legacy storage is absent.
It is install-only and rejects dry-run, force, config-version and deprecated-cleanup combinations.
After relocation, dashboard review and ordinary install retain their separate admission requirements.
...
Coordination claims are not generic cleanup candidates. `.goat-flow/state/locks/*.claim` remains fail-closed until owner-checked release or explicit recovery after an operator confirms no active writer; elapsed time never authorizes removal. A crashed recovery may leave a matching `.recovery` guard; an operator must confirm no recovery or writer is active before removing that guard.
...
| `project.save` | existing | `dashboard-session-trace` | server | Project/favourite/add/archive/restore counts | No project-list body | Project-list continuity |
| `project.remove` | legacy (not emitted) | `dashboard-session-trace` | server | Historical removed-project count | No removed path list | Backward-compatible validation of older local event logs |
