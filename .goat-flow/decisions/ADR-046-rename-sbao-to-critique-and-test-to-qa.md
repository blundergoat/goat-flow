...
- **Positive:** `/goat-critique` aligns the name with the trigger verb users already type. The router row that used to read "review vs sbao vs plan" now reads "review vs critique vs plan", which matches the user's natural verb instead of forcing them to know the acronym first (`.goat-flow/skill-preamble.md:85-86`).
...
- `.goat-flow/skill-preamble.md:71-73,85-86`
...
- `src/dashboard/preset-prompts.ts:92-103,152-155`
...
<details>
<summary>Drafting notes (remove before committing)</summary>

- **Files I could not read:** `.claude/skills/goat-sbao/SKILL.md` and `.claude/skills/goat-test/SKILL.md` no longer existed in the working tree because M07 execution had already renamed them. I used `.claude/skills/goat-critique/SKILL.md` and `.claude/skills/goat-qa/SKILL.md` as the current equivalents.
- **Facts marked INFERRED / session-sourced:** the "final shortlist convergence" sentence comes from the current-session decision context, not from a committed repo artifact with line references.
- **Format choices for human review:** I matched ADR-045's top matter (`**Status:**`, `**Date:**`) and section tone, but added explicit `## Alternatives considered` and `## Related decisions` sections because this decision needs shortlist and lineage detail.
- **Human verify before commit:** confirm whether citing current renamed skill paths is acceptable for the historical friction bullets. The working tree currently keeps the `SBAO` glossary row as a historical alias.
