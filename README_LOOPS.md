...
4. **Graduation** - when a recorded mistake happens again, the entry gets a line-start recurrence label such as `**Recurrence update` or a higher `**Incident count:**`. `goat-flow stats` lists an active entry as a **graduation candidate** once it reaches two incidents - the larger of its declared count and one plus its recurrence labels - and shows each candidate's incident count. The migration path for each candidate: promote the prevention to a structural gate (preflight check, CI step, deny pattern) or resolve the entry. Candidates are report-only - never a `--check` failure - so the signal cannot decay into ignorable warning noise.
