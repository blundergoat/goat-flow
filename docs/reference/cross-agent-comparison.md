...
The core system transfers without modification: the execution loop (full behavior in `docs/system-spec.md`), autonomy tiers, Definition of Done, footguns file, lessons file, router table, essential commands, and the eval concept. These aren't Claude Code features -- they're workflow patterns that work on any agent that reads a root instruction file.
