...
`hard`, `limited`, and `soft` are positive local strength labels; `missing` means the inspected local surface is absent or failed; `unknown` means the audit cannot infer the broader capability. A `hard` row is rejected unless local static or runtime evidence exists. Static test fixtures prove this contract shape only, never runtime agent behavior. Audit is static by default. For deeper configured-command proof after inspecting and trusting the checkout, use `goat-flow hooks verify . --agent <id> --scenario <deny-hook|post-turn-hook|gruff-hook|all> --trusted-target`; `all` refreshes every group in one command. It runs fixed offline inputs through that agent's exact registered command, but does not exercise external-agent hook delivery or model visibility.
