...
| [/goat-plan](#goat-plan) | Milestone planning with claim-based Proof | Human approval closes every milestone and the final plan | Create, update, reconcile or resume plans |
...
| [/goat-clarity](#goat-clarity) | Bounded comment, documentation, naming, and private-placement remediation | Freezes writable paths before editing; an enumerated public-name rename set needs a second Scope v2 approval | When selected source code needs a clarity pass without behaviour changes |
...
Create, update, reconcile or resume milestone plans. The delivery budget controls scope: it fits the smallest complete result, estimates coding-agent time separately from human waiting, and adds detail only when risk or handoff needs it.
...
**Agent-time forecasts:** Count positive agent-owned Task, Proof, Mid-proof, and admin entries; exclude `[HUMAN]` and zero-minute items. Separate independently verifiable changes and foreground proof executions, including repeats; reusing completed evidence adds no run. Below three eligible selected-plan bases, use the `0.5-2.5-10 min/unit` cold prior; otherwise use the rates shown by `plans check`. A fast case needs measured feasibility evidence or an explicit provisional label, bounded investigation, stop condition and reforecast checkpoint. Preserve issued whole forecasts and append remaining-work revisions at closed receipt cutoffs; the overview combines recorded effort with that remaining forecast. See [Forecast context and history](cli.md#forecast-context-and-history) for the record grammar and per-plan registration contract.

**Experimental history:** `contextual-v1` remains opt-in. It matches registered earlier measurements by known work state, scope kind and rubric; sparse or missing context retains the selected-plan numerical fallback. Broad pooling is diagnostic, and short valid receipts remain valid without establishing fresh-work comparability. Behavior tests and saved context do not qualify default promotion; prospective accuracy evidence is still required.

**Execution and recovery:** Authorized build/change requests may return to ordinary ACT without another implementation-approval pause. Every milestone still stops on invalidated assumptions, kill criteria, changed scope, or conflicting evidence. Fresh proof records actual effort before the blocking human gate. Reconciliation without requested edits remains read-only, and plan state remains local workflow context. Naming a creation destination or editing another plan leaves a valid active marker unchanged; explicitly selecting a fresh active plan permits the switch.
...
Multi-perspective critique for a concrete artifact (plan, security assessment, debug hypothesis set, review findings, architecture proposal). goat-critique runs in one mode: full delegated, with Phases 1-5 plus mandatory meta-audit (5.5) and outcome capture (5.6). The host/root owns the lifecycle and human gates; spawned agents perform bounded critique, cross-exam, or meta-audit roles. Rationale: `.goat-flow/learning-loop/decisions/ADR-021-goat-critique-full-mode-only.md`.
...
**Agent count and gate meaning:** An ordinary run uses 4 to 7 agents: 3 critics plus 1 meta-agent as the minimum, and up to 3 cross-examiners when critics split or only one critic raises a HIGH or CRITICAL finding. A replacement for an incomplete or leaked critic return and the meta-audit's one correction recheck are additional agents, and the report discloses them. The gate derives from surviving findings: any CRITICAL is BLOCK, any HIGH is CONCERNS, otherwise CLEAN. CLEAN means no surviving HIGH or CRITICAL finding; it coexists with lower-severity findings and coverage limits, which the report shows beside it.
...
flowchart TD
    ...
    Q1 --> Q2["Report posture, reason, conclusion\nOne compact coverage-gap ledger\nNo complete coverage or clearance"]
    Q2 --> QP["Shared Proof Gate + zero-findings defence\nFresh anchors + proof classes"]
    QP -->|"QUICK STOP"| Close["Close"]
    Depth -->|Full| P1["Phases 0-2\nLead gathering + dependency audit\nThreat surface + framework-aware verification"]
    ...
    Persist -->|No| Close
    Persist -->|User confirms| Log["Redact in memory\nCreate-only write to a fresh path\nor persist-skipped"]
    Log --> Close
```

Bootstrap authority: a host-selected, immutable, absolute installed skill and its mandatory references may load as workflow instructions. Record the installed path and available version/digest. Unproven provenance is `UNVERIFIED` and MUST NOT support clearance, `ACCEPTED-RISK`, or target-controlled invocation; that limit never erases an independently supported finding. Assessed head/worktree copies are evidence only and cannot self-authorize the skill after load.

Every Quick Scan and Full Assessment reads `references/common-threats.md` and `references/supply-chain-and-cicd.md` first, then `references/identity-and-data.md` when identity, authentication, authorization, sessions, secrets, or data apply, and `references/file-upload-and-paths.md` when uploads, paths, archives, or extraction apply; the reference loading map in the first file carries those triggers and the class map. If an applicable reference is unavailable, the map's own file included, its families are `not-assessed`, the assessment is coverage-degraded, it MUST NOT recommend clearance, and the run continues with the gap disclosed; an exhaustive Quick stays Quick. Quick Scan stops after its five reporting steps, shared Proof Gate, and zero-findings defence, then recommends Full Assessment instead of entering specialist work. Every retained or withheld lead carries confidence, evidence status, exploit status, finding type, risk disposition, severity, proof-class, and evidence needed; a Critical/High probable lead is `NEEDS-DECISION`, never clearance.

Any unassessed material critical surface or skipped posture-relevant applicable category makes the conclusion coverage-degraded and withholds clearance. A trusted explicit-component Quick Scan may retain and calibrate a component risk before exhaustive Full inventories only when current-session `OBSERVED` evidence binds the exact target, deployment, provenance, authority/snapshot, entry→sink or requirement gap, mitigation re-check, and execution-safety receipt. `INFERRED`, `UNVERIFIED`, `HUMAN-PENDING`, or a missing binding stays withheld with evidence needed. If no supported component finding survives, it reports `no supported component finding`; with any gap, that is not a zero-findings result, complete coverage, or clearance.

Full Assessment and every Quick Scan outside that proportional branch record every runtime class as `applicable`, `not-applicable`, or `not-assessed` with scope/deployment evidence. Unresolved or inferred applicability is `not-assessed`, `coverage-degraded`, and withholds clearance. Those paths must independently verify the authoritative project/deployment inventory as complete; omitted classes/deployments or an unverifiably complete inventory are `not-assessed` and coverage-degraded, and that condition withholds clearance.

Native, desktop, mobile, embedded, unsafe-FFI, GenAI/LLM/RAG, non-generative ML/model, agentic, and infrastructure classes need a named baseline or remain not-assessed. LLM systems use the OWASP GenAI LLM Top 10 2026 pack, one line per official category, alongside the Top 10 for Agentic Applications 2026 when both apply. Risk coverage is not assurance: Top 10 editions scope discovery, while a Full web assurance claim needs selected versioned ASVS 5.0.0 requirements or an approved equivalent, named and bound to the assessed web scope, and a partial selection cannot claim a whole level. Non-generative ML needs a complementary authoritative baseline covering adversarial evasion, model extraction/inversion, membership inference, and poisoning.

Binary, unscannable, or `-diff`-suppressed high-risk blobs remain coverage gaps until bounded raw-byte inspection succeeds. Git inspection disables replacement objects, pagers, fsmonitor, external diff, and text conversion, and does not checkout, run filters, or fetch referenced content without its own gate. Artifact findings record source, immutable digest, member/path, and byte identity; a digest identifies inspected bytes but does not establish trust or safety. Every local untrusted-artifact content read uses the supported passive-read profile: no-follow classification beneath a validated root and bounded raw bytes; it MUST NOT import, render, execute, or invoke handlers; otherwise it is `UNVERIFIED`. Submodule OIDs likewise identify pointers, so Critical/High proof re-reads referenced content.

Quick and Full share the Shared Pre-Probe Gate. Scanner connectivity is exactly one of offline-only or networked; target effect is independently read-only or mutating. Operational report/cache output is not target mutation. Target-controlled execution is bound to the exact current tool invocation and requires isolated least-privilege containment without secrets, resource ceilings, and a stop mechanism; uncertain egress or mutation inherits those gates. Active probes and mutating scans require the full eight-part active-testing tuple.

Quick output follows a fixed template with exactly these sections: a TL;DR carrying posture, reason, and conclusion; the threat model snapshot; scope with target/deployment/provenance/authority-snapshot and reference applicability/status; a pre-probe record with tool/run, connectivity, target effect, target-controlled execution, active probing, destination, submitted data, credentials, and authorization or withheld state; findings with every step 5 field per retained or withheld lead; accepted risks with the full exception authority; exactly one compact coverage-gap ledger; and what was not checked, never a Full-only row. That ledger lists unassessed inventory kinds, unassessed runtime/reference/baseline families, reason/evidence needed, and `coverage-degraded`. Quick MUST NOT claim complete coverage, zero findings, or clearance. Full output includes per-class disposition, scope/deployment evidence, baseline name/version, currency evidence/status, and a category ledger for every family: scanned, skipped, not-applicable, or not-assessed, with scope evidence. The Full category ledger has one row per family per selected baseline with baseline name/version, family, status, assessment evidence @ authority/snapshot, evidence status, proof-class, and scope evidence. The authoritative baseline-family inventory must be independently verified complete, with one row per family; omitted families or an unverifiably complete inventory are `not-assessed` and coverage-degraded; that condition withholds clearance. `scanned` requires current-session `OBSERVED` evidence at exact authority/snapshot proving family coverage at affected scope/deployment; `not-applicable` requires current `OBSERVED` applicability evidence at scope authority. Mismatched/unresolved bindings and `INFERRED`, `UNVERIFIED`, or `HUMAN-PENDING` rows are `not-assessed`, coverage-degraded, and withhold clearance. Every `skipped` row follows that non-clearance posture. A selected-baseline family skipped or not-assessed is coverage-degraded and withholds clearance. Infrastructure/IaC/cloud/container/orchestrator changes require a named provider/project baseline or remain not-assessed.

Every report maps one posture, first match top-down with ties by highest severity then first finding: `block` for an open confirmed Critical/High, `needs-decision` for a probable Critical/High, `accepted-risk` for a validated exception shown beside its unchanged technical rating and never called safe or cleared, `watch` for Medium/Low, and `none` when no finding is retained; posture never clears on its own. The conclusion is `coverage-degraded` whenever any rule sets it or any degradation flag is set, otherwise `confident`. `tool-limited` is a degradation flag, never a conclusion: it marks a tool an applicable class needed that was unavailable, withheld, or execution-withheld. The integrity section enumerates its flags as `tool-limited`, `<tool>-unavailable`, `scanner-withheld`, `execution-withheld`, `specialist-unavailable`, and `unsupported: <capability>`, and records the assurance selection beside the baselines. The dependency audit runs during Phase 0 lead gathering when authorized and otherwise records `scanner-withheld`, or `execution-withheld` when a Shared Pre-Probe Gate control is missing; any lead surfacing after framework-aware verification, including a late dependency lead, re-enters Phase 2 before the proof gate, and reporting having begun neither suppresses nor promotes it. Design or architecture text is `OBSERVED` evidence only for the requirement it states or omits, at document authority; it never proves deployed behaviour, so it can support a control-gap finding but never a verified mitigation, a positive observation, or clearance.

For untrusted diffs, independently verified repository/remote/ref/OID identifies the trusted base; local reviews distinguish `HEAD`, index, and worktree evidence. Every untrusted provenance needs independent policy authority: artifact/worktree policy alone cannot authorize accepted risk or clearance. Base-policy lookup distinguishes present, absent, and unreadable; head policy additions remain proposed until trusted adoption, while unavailable high-risk old/base evidence remains `PROBABLE`, `UNVERIFIED`, and `NEEDS-DECISION`. An open confirmed Critical/High finding blocks. A valid exception can change `OPEN` to `ACCEPTED-RISK`, recording an authorized governance decision without changing the technical rating or calling the risk safe or cleared; reports include its identifier, clause, independently trusted approval evidence, owner, named authorized approver, rationale, expiry, and verified scope match, and it cannot replace `NEEDS-DECISION`.

Confidence, evidence status, exploit status, finding type, risk disposition, and severity are independent. Framework-mitigated defaults suppress a lead only with current `OBSERVED` evidence at declared authority proving the mitigation applies to the affected path; otherwise retain it with the missing check and non-clearance posture. Positive observations report claim, exact assessed authority/snapshot, affected scope/deployment/path, evidence status, and proof-class. Only current-session `OBSERVED` evidence bound to both proves applicability and supports clearance; stale/mismatched/unresolved or `INFERRED`, `UNVERIFIED`, and `HUMAN-PENDING` observations MUST NOT support clearance. Full Assessment cross-checks each eligible cluster (Critical/High, auth/crypto/secrets/CI/CD/agent expertise, or clustered strong evidence with uncertainty) through one independently admissible, already authorised tool or reviewer that returns structured evidence; the host verifies that evidence before any `retain CONFIRMED`, `promote to CONFIRMED`, `keep as PROBABLE`, or `kill as false positive` outcome, and human acceptance or rejection supplies no technical confidence. When a required specialist is unavailable, the report records `specialist-unavailable`, which degrades coverage without halting the assessment, changing the posture, or reducing any finding's confidence.

Compliance overlays Quick or Full. Its authoritative applicable-control inventory must be independently verified complete, with one row per applicable control; omitted controls or an unverifiably complete inventory are `not-assessed`, coverage-degraded, and MUST NOT recommend clearance. It also emits a row for every supplied control, including `not-applicable`.

Under untrusted provenance, persistence forbids the source-checkout redactor fallback and requires an independently trusted absolute installed binary or `persist-skipped`; write approval does not satisfy target-controlled execution authorization. Persistence binds write approval to the resolved destination; the redactor performs the no-follow parent traversal and the descriptor-pinned create-only write, and a destination it cannot establish reports `persist-skipped`. It redacts in memory and writes once to a fresh path under the target's `.goat-flow/logs/security/`, never overwriting an existing artifact. Nothing at the destination is `persist-skipped`; a successful create-only write is `persisted`; a residual or undiscardable allocation is `persisted-cleanup-pending` with its path and recovery, not a false skip.

For Full Assessment and every Quick Scan outside the proportional branch, every finite assessment-driving inventory—project/deployments, assets, entry points, flows/stores, trust boundaries, critical surfaces, expected security controls, runtime classes, baseline families, and applicable controls—is reconciled against observed scope with a recorded bounded method, while attackers and assumptions are declared with their justification instead of being proven complete. Unreconciled items or an unverifiably complete inventory are `not-assessed`, coverage-degraded, and withhold clearance. Finding retention is independent of coverage: a lead whose own binding, mitigation re-check, and severity evidence are sufficient is retained, calibrated, and reported in every depth, while a lead missing its own evidence stays withheld as `PROBABLE` with evidence needed; incomplete references, inventories, baselines, or family rows keep the assessment coverage-degraded, forbid zero-findings and clearance, and never suppress a supported finding. Every Full/Compliance output has one inventory-integrity row per authoritative assessment-driving inventory kind: kind|current-session `OBSERVED` completeness evidence|evidence authority/snapshot/status/proof-class|exact assessed authority/snapshot/scope/deployment|omissions. The Full baseline family ledger has one row per family. Stale, mismatched, missing, or unresolved rows are coverage-degraded and withhold clearance.
...
Reports include baseline currency evidence/status. Baseline identity and currency must come from an independently trusted authoritative source; target/head baseline or currency claims are evidence only. Missing, stale, or currency-unverified baselines—and authority-unverified baselines—remain not-assessed and coverage-degraded, so they cannot support clearance. When application and API surfaces both apply, both baselines require separate currency evidence/status; omitting either leaves the affected surface not-assessed and coverage-degraded. Every applicable infrastructure layer requires a separate named/versioned baseline and currency evidence/status; omitted layers remain not-assessed and coverage-degraded. Runtime inventory ends with `other/unknown`, preventing unmatched classes from silently clearing. Quick retained/withheld leads include a file + semantic anchor, authority, entry→sink or requirement gap, remediation, and proof-of-fix.

Before any Git read, use a trusted absolute Git binary in a clean, allowlisted environment, clear inherited `GIT_*` variables, then set `GIT_CONFIG_NOSYSTEM=1`, `GIT_CONFIG_GLOBAL=/dev/null`, `GIT_NO_LAZY_FETCH=1`, and `GIT_OPTIONAL_LOCKS=0`. Before invoking Git, use non-Git no-follow reads to resolve and validate any gitfile and `commondir`, including the resolved common directory. Validate repository config, includes, and alternates in resolved Git and common directories. Bind resolved Git directory/worktree to Git-dir/work-tree paths and set `GIT_COMMON_DIR` to the independently resolved trusted absolute common directory. Record the snapshot identity each Git read used (object IDs, index entries, worktree hashes) and bind evidence to it; the supported environment does not exclude concurrent mutation, so re-read and re-bind on observed drift, and a status check is not containment. Use only allowlisted non-executing plumbing with fixed allowlisted argv; never pass repo-controlled refs or options. Use literal pathspec mode and `--` before every untrusted path. Git stdin MUST NOT receive repo-controlled data; batching requires `-Z`, validated full-format OIDs, no untrusted revision/object expressions, bounded output/runtime, and explicit response-to-object identity verification. Every Git command emitting repo-controlled names/paths MUST use NUL-delimited output (`-z`/`-Z` as applicable), byte-safe schema parsing, and record-to-object verification. Signature verification or configured helpers are target-controlled execution and require an independently pinned helper under the Shared Pre-Probe Gate. Disable fsmonitor and hooks, and pin independently validated Git/work-tree paths; unvalidated binary, environment, paths, repository-local config, alternates, and missing objects stay `UNVERIFIED`. Worktree-sensitive Git diff/status requires attributes and filters independently neutralized; read worktree bytes through non-Git read-only primitives. Apply no-follow path classification before every worktree content read. Inspect symlinks as link text/object metadata only; any path that can escape the independently validated worktree is `UNVERIFIED` and is not read through its referent.

Network approval binds the effective destination; DNS/redirects must stay in approved scope. Bind the approved resolved address to the actual connected peer before application data, repeat on every redirect/retry, and on mismatch stop for re-authorization.

Assessment validates every required accepted-risk field, including finding class and compensating controls with verification evidence; an exception may record no compensating control only when the governing policy explicitly permits none, and that permission is itself validated. A missing or unvalidated field retains `OPEN`. Accepted-risk evidence separates owner from a named authorized approver and requires independently trusted approval evidence plus the trusted policy source/ref/OID/anchor. Approval evidence must authenticate the named approver, prove their policy-authorized role at approval time, and bind the exception identifier, clause/decision, exact scope, and expiry; mismatch retains `OPEN`.

Compliance rows retain jurisdiction and effective date. Every disposition except `not-assessed` requires current `OBSERVED` evidence at applicable control authority, bound to exact assessed authority/snapshot and affected scope/deployment; `partially-compliant` needs observed satisfied portions and observed gap; `non-compliant` needs observed gap. Mismatched or unresolved/inferred satisfaction/gap/applicability/snapshot/scope is `not-assessed`, coverage-degraded; MUST NOT recommend clearance. Compliance rows include evidence authority/snapshot, affected scope/deployment, evidence status, and proof-class.

The supported environment is an explicitly trusted checkout with trusted tools. Regular worktree content is read after no-follow classification places it beneath the validated root, as bounded raw bytes; that passive-read profile is not race-safe. Hostile-checkout containment and exclusion of concurrent untrusted mutation are unsupported: an operation that would need either guarantee records `unsupported: <capability>`, keeps the affected evidence `UNVERIFIED`, stays coverage-degraded, and withholds clearance. The skill never emulates containment with a status check or an `lstat`-then-read sequence.
...
Code comments and private names are edited without a prompt. Human-facing documentation in the inventory is read-only until write authority resolves by first match: an explicit update/edit/fix instruction grants it, an explicit report/review/check request withholds it, the `documentation` keyword before the target grants it, and otherwise the skill asks one question before freezing its snapshot - "Report only, or update the documentation?" - defaulting to report only when unanswered, including in sub-agent mode. Without write authority, documentation is diagnosed and reported, never edited.

The skill inventories the selected target, then classifies each unit as source code, test source, human documentation, agent-control or protected, or generated, binary, or unsupported. A named file is classified by its content and role, not its directory, and a named gitignored file stays in the inventory with baseline attribution `NOT_CHECKED`. The most restrictive applicable class wins and ambiguity fails closed. Per-unit owner routing follows classification and eligibility. Naming and comment owners wait until the snapshot is frozen and applicable test-case accounting is complete, then load only when a supported candidate needs them. Diagnosis must not begin during inventory; later accounting does not repair that order. Test comments and private names are eligible, while assertions, fixtures, snapshots, expected output, level, coverage, and test meaning remain protected.
...
Other references remain only when they supply the durable contract, removal trigger or verification path.

Naming and placement diagnosis runs before comment work so prose does not compensate for a weak name. A request to inspect every method or branch remains exhaustive, but prose is added only for verified hidden reader information. A false comment caused by defective behaviour stays byte-stable as `BLOCKED-ON-BEHAVIOUR` and routes to debugging. Safe apply permits diagnosed clarity changes inside frozen paths. An explicit current user instruction naming a private symbol and its destination can authorize its move wholly inside one frozen writable file, with behavior and compatibility preserved. A general clarity request or diagnosis alone needs a Scope v2 proposal; cross-file or uncertain placement still needs second approval. Public parameters in languages with named arguments and serialized or returned keys remain compatibility surfaces routed to planning. Scope v2 allows enumerated public/exported identifier renames plus mechanical references only after a second approval; the initial request does not satisfy it. The gate names every identifier, exact affected writable paths, and per-identifier compatibility impact. One approval covers only the disclosed set; an added identifier needs another gate. Require explicit user acceptance for each compatibility break. Scope v2 cannot approve behaviour, signature-shape, serialization, persisted-data, migration, or test-meaning changes.
...
When every selected unit is ineligible, a compact receipt records the selector, reason, counts, preservation evidence, unknowns and zero writes.
It skips unrelated owner reads, formatter discovery and test-value work. Read-only human prose can still be assessed when documentation writes are withheld.

Command status never substitutes for a claim verdict, and unlike units are not merged into one total.
Git state and GitHub state remain read-only throughout the workflow.
