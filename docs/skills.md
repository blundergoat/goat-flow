...
**Agent-time forecasts:** Count positive agent-owned Task, Proof, Mid-proof, and admin entries; exclude `[HUMAN]` and zero-minute items. Separate independently verifiable changes and foreground proof executions, including repeats; reusing completed evidence adds no run. Below three eligible selected-plan bases, use the `0.5-2.5-6 min/unit` cold prior; otherwise use the rates shown by `plans check`. A fast case needs measured feasibility evidence or an explicit provisional label, bounded investigation, stop condition and reforecast checkpoint. Preserve issued whole forecasts and append remaining-work revisions at closed receipt cutoffs; the overview combines recorded effort with that remaining forecast. See [Forecast context and history](cli.md#forecast-context-and-history) for the record grammar and per-plan registration contract.
