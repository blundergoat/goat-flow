...
Accepted-risk evidence separates owner from a named authorized approver and requires independently trusted approval evidence plus the trusted policy source/ref/OID/anchor. Approval evidence must authenticate the named approver, prove their policy-authorized role at approval time, and bind the exception identifier, clause/decision, exact scope, and expiry; mismatch retains `OPEN`.
