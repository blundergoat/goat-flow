...
Bootstrap authority: the invoking host, before loading goat-security or its mandatory references, must bind them to an independently trusted, immutable, absolute installed source with version and digest. Assessed head/worktree copies are evidence only. Otherwise withhold invocation and clearance; a loaded skill cannot establish its own authority after load.
...
Changed tests in a PR or uncommitted selector, and every test in selected folder or file test-source units, receive a report-only value pass through `test-selection.md`. The test-selection record assigns each assessed test `KEEP`, `CONSOLIDATE`, `MOVE LEVEL`, `PRUNE CANDIDATE`, or `UNRESOLVED` after tracing its production behaviour and overlap. A manifest checkpoint counts cases before broader diagnosis and caps provider evidence batches at 20 cases, so incomplete cases remain anchored `UNRESOLVED` rows instead of disappearing from the ledger. Recommendations never authorize assertion, fixture, level, coverage, or test-meaning changes.

The skill freezes a Target Scope Snapshot before its first edit. A matching PR checkout can admit eligible clarity edits. When authenticated provider evidence is available but the local repository or head differs, the remote report-only lane freezes the provider base/head snapshots with no writable paths, no formatter or runtime claim, and a final head-drift check. `uncommitted files` uses byte-safe staged, unstaged, and untracked non-ignored path inventories; folder and file selectors must remain inside the repository without symlink traversal. The scoped `references/target-scope-and-evidence.md` reference owns inventory, content-digest drift, formatter capability, command status, claim verdict, and like-unit receipt mechanics.
