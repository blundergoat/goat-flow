...
New prompts preserve up to five recommendations in `improvements`, each with `category` (`defect`, `qualification-gap`, `maintenance`, or `design-opportunity`), `summary`, `action`, `evidence`, and nullable `file`. Summary text is capped at 240 characters; action and evidence at 1000 each. Text must be non-empty and single-line. An empty array means none were proposed; omission means recommendations were not recorded. This optional field remains compatible with earlier report writers.

`assessment_context.workspace_snapshot` optionally retains `start` and `end` fingerprints from the version-matched `review snapshot` command using an `area` request for roots `["."]` and `sample: null`. Capture both endpoints with the same request; null means capture was unavailable. The fingerprint covers the selected raw-file inventory and excludes ignored untracked files and nested repositories, so re-read any ignored evidence separately. Present endpoints must be non-null and equal to claim complete grounding. These are assessor-recorded values, not proof from the dashboard launcher.
