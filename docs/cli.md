...
`quality diff` also returns `comparisonWarnings` and prints each as `Comparison limit:`. Missing provenance, incomplete grounding, different revisions, rubric or project scope, and missing or different workspace snapshots limit what a score delta can prove. Matching reviewer-recorded snapshots still do not attest that both assessments ran the same checks.
...
Current reports require `improvements`, preserving up to five recommendations, each with `category` (`defect`, `qualification-gap`, `maintenance`, or `design-opportunity`), `summary`, `action`, `evidence`, and nullable `file`. Summary text is capped at 240 characters; action and evidence at 1000 each. Text must be non-empty and single-line. An empty array means none were proposed; omission is accepted only when reading historical reports and means recommendations were not recorded.

Current reports require `assessment_context.workspace_snapshot`, retaining `start` and `end` fingerprints from the version-matched `review snapshot` command using an `area` request for roots `["."]` and `sample: null`. Capture both endpoints with the same request; null means capture was unavailable. The fingerprint covers the selected raw-file inventory and excludes ignored untracked files and nested repositories, so re-read any ignored evidence separately. Endpoints must be non-null and equal to claim complete grounding. These are assessor-recorded values, not proof from the dashboard launcher. `project_revision` must be a full lowercase Git object ID or null. Provenance, rationale and recommendation text reject line breaks, terminal controls and bidirectional formatting controls.

History readers accept regular files up to 2 MiB inside the selected project. Linked paths, special files and oversized reports are skipped with warnings. Recall and `learn new` citation reads use the same regular-file checks with an 8 MiB evidence limit.

New `runtime-probe` and `mixed` findings must retain `evidence_command`, `evidence_exit_code`, and `evidence_summary` from the same completed tool call. Non-zero exits are valid evidence; missing exits cannot be invented. Historical reports with weaker evidence remain readable through compatibility parsing.

```bash
npx @blundergoat/goat-flow@latest quality save . <<'JSON'
{"report_kind":"goat-flow-quality-report","goat_flow_version":"<current-version>","agent":"claude","project_path":"<absolute-project-path>","run_date":"YYYY-MM-DD","audit_status":"pass","scope":"framework-self","rubric_version":"<current-version>","quality_mode":"skills","prior_report_id":null,"assessment_context":{"project_revision":"<git-head>","working_tree_state":"clean","grounding_status":"partial","unverified_probes":["workspace snapshot not captured in this shape-only example"],"score_confidence":"low","workspace_snapshot":{"start":null,"end":null}},"scores":{"setup":{"total":0,"accuracy":0,"relevance":0,"completeness":0,"friction":0},"system":{"total":0,"usefulness":0,"signal_to_noise":0,"adaptability":0,"learnability":0}},"score_rationale":{"setup":{"accuracy":{"evidence":"<observed evidence>","deduction":"<reason or no deduction>"},"relevance":{"evidence":"<observed evidence>","deduction":"<reason or no deduction>"},"completeness":{"evidence":"<observed evidence>","deduction":"<reason or no deduction>"},"friction":{"evidence":"<observed evidence>","deduction":"<reason or no deduction>"}},"system":{"usefulness":{"evidence":"<observed evidence>","deduction":"<reason or no deduction>"},"signal_to_noise":{"evidence":"<observed evidence>","deduction":"<reason or no deduction>"},"adaptability":{"evidence":"<observed evidence>","deduction":"<reason or no deduction>"},"learnability":{"evidence":"<observed evidence>","deduction":"<reason or no deduction>"}}},"findings":[],"refuted_candidates":[],"improvements":[]}
JSON
```
...
The active cap defaults to one. A valid `--max-active <n>` overrides `plans.maxActiveMilestones`; `--band-quantiles <low,high>` overrides `plans.forecastBandQuantiles` (default `[10, 90]`). A physically contained canonical `.goat-flow/plans/` path uses its owning project’s `.goat-flow/config.yaml`, including when the selected project is outside the working directory. External or symlink-escaped plan paths use defaults. Supplying both flags avoids reading project config; otherwise malformed canonical config exits `2`. Cap flags require positive safe base-ten integers without leading zeroes; config caps require positive safe integers. Quantile flags use comma-separated decimal numbers and config uses a numeric pair; both require `0 < low < 50 < high < 100`, allowing asymmetric pairs. These flags apply only to `plans check` and never write configuration.
...
Both modes read milestone files only; authors separately reconcile ISSUE delivery bands with the headline and map Requirements to outcomes and proof. Strict validation checks supplied deterministic structure, not planning judgment. It does not infer risk level or require assumptions, manual proof, rollback, Boundary Notes, or other conditional fields. Default mode remains legacy-compatible, and neither mode reconstructs approval history or evaluates whether proof is semantically sufficient.
...
Legacy point-estimate plans need neither `Forecast basis:` nor `Forecast range:`. Supplying a basis requires its derived range in both default and strict modes. A supplied basis counts positive agent-owned Task, Proof, Mid-proof, and admin entries while excluding `[HUMAN]` and zero-minute items. Its positive low-likely-high rates must be ordered; multiplying them by the unit count derives the range by flooring low (minimum one), rounding likely, and ceiling high. The declared unit count, range, and `Effort estimate` headline must all agree. A range without a basis retains its legacy check: `low <= likely <= high`, with likely equal to the headline.

Calibration output is informational and never affects the exit code. It retains estimate-to-Actual ratios and separately divides raw receipt seconds by matching, countable bases. Only `complete` milestones with `measured` Actuals qualify - `complete` is the human ratification signal, so `human-verification-pending` never calibrates a forecast. Below three eligible work-unit samples, authors use the conservative `1.0-2.5-6 min/unit` cold-start prior. At three or more, the report shows each sample, the median, and the selected interpolated historical quantile rates (p10/p90 by default) at index `(n - 1) * p` of the ascending sample. These historical quantiles do not promise future coverage. Local rates are rounded to two decimals before deriving whole-minute bounds; an unfinished basis receives `reforecast required` only when its low, likely or high bound differs. When local history runs under one minute per unit, positive whole-minute items cannot sum to the likely it implies, so the advised likely is floored at `1.00 min/unit`, provided the high rate is at least `1.00`; the line says `likely floored` and names the measured rate, and a basis that copied the floored rates receives no further advice. A changed work-unit count remains a validation error. The CLI reports these results but never rewrites the plan.

The separate rule-coverage report replays the selected pair using each eligible milestone’s last closed completion epoch. Equal-time milestones use only strictly earlier groups and enter training together afterward. Predictions require eight earlier eligible receipts and use the published two-decimal rates with the same whole-minute derivation. `scored k of n` distinguishes scored predictions from the full current work-unit cohort; per-file lines disclose insufficient history or missing usable completion time, which also excludes training. Empty scored history says `no score`. The report lists completion order, prior counts, rates, derived bounds and outcomes. Narrower quantiles preserve the median and produce nested intervals on identical scored IDs; observed coverage can stay equal or fall. Historical percentile spans do not promise future coverage. Authored-band coverage remains separate, and the command never rewrites existing forecast fields.

Calibration eligibility uses raw receipt seconds and a positive estimate in both pools. A missing or stale countable basis excludes only the work-unit pool. When completed records exist, the report names each completed-record exclusion and its affected pools, and summarizes non-complete records in one line; plans without completed work retain the uncalibrated summary. A positive receipt that rounds to zero minutes stays eligible and receives a factual note. Short closure work may differ from implementation work, and task decomposition changes minutes per unit; neither a low rate nor a rounded zero proves a late timer or invalid measurement.
...
Unit growth reports work added after forecasts, using saved forecast records only. For `complete` milestones whose records the parser accepted, under either forecast method, it counts the units in each original record, the distinct item IDs that later revisions registered as added, and the share added; an item added, removed and added again counts once. A `complete` milestone whose checklist count differs from its forecast basis, and that saved no revision, is counted separately with no direction, because that work may have been added, removed or re-split. Additions that were never registered leave no trace, so the reported growth is a floor. The `unit growth:` line is omitted when neither case occurs.
...
An explicitly authorized reset preserves exact prior receipts and Actual inside fenced `Reset history`, removes their live representations, reopens scoped Tasks/Proof/Mid-implementation proof/Exit checkboxes, clears current Actual and stale Status reason, then sets `not-started` and validates. A paused receipt still counts as live metadata and cannot remain in that state. Human acceptance alone never reopens timing. The [planning lifecycle checklist](../workflow/skills/goat-plan/references/milestone-examples.md#timing-receipts) owns the operation order and approval boundaries.
...
`--dry-run` prints a read-only preview of the install write set as text or stable `goat-flow.managed-setup-preview.v2` JSON. Every row carries a repository-relative path, its manifest ownership, a state, and the proposed action and reason. Exact-copy `system-owned` templates are classified as `unchanged`, `template-changed`, `local-preserved`, `both-changed`, `added`, `adopted`, `removed`, `missing`, or `unmanaged`. Destinations with no package template use four further states: `user-seeded` for a user-owned file install may create once, `user-preserved` for user-owned content install keeps, `user-migrated` for a user-owned file install edits in place, and `regenerated` for a generated file install rewrites from project state. Every row also carries an `authority` decision - `not-required`, `granted-managed`, `granted-path`, `granted-user-owned`, `withheld`, or `refused-path-safety` - resolved against the same flags apply would use, so dry-run accepts installation authority and migration flags and still writes nothing; the separate `--migrate-state-only` action cannot be combined with it. A blocked preview exits 1; invalid flags exit 2. `--output` is the only optional dry-run write and writes the requested report, not setup state or installed files.

The comparison uses SHA-256 hashes only. `.goat-flow/state/install/managed.json` is the sole project-wide baseline: each managed path has one expected hash, generation, and provenance. The same file carries hashless verified-agent receipts that bind a package version to an exact path and row-generation set. A receipt remains confirmed only while its package version, path set, referenced generations, regular target bytes, and cutover marker still match. Any mismatch makes the receipt stale, so it cannot select an installed agent; the baseline row remains available to distinguish a preserved local edit from a later template change.

Installation records and write locks share the gitignored `.goat-flow/state/` parent: `install/` holds baselines and cutover markers, and `locks/` holds exclusive write claims. Before upgrading a project with `.goat-flow/install-state/` or `.goat-flow/write-claims/`, stop every writer and upgrade every goat-flow process. An applying `goat-flow install` moves those directories to the new layout without rewriting their records; dry-run can read legacy evidence and never moves it. Outstanding claims, unsafe paths, or an occupied destination stop migration without merging or overwriting either directory. Use `claims inspect` and explicit abandoned-claim recovery before retrying when necessary. An interrupted move can be resumed by another install.

If install asks for policy review but dashboard Sync reports legacy local state, stop and upgrade every writer, then run:

```bash
goat-flow install <project-path> --agent <id> --migrate-state-only
```

This explicit action moves only the old bookkeeping directories; it does not install hooks, change policy choices or record a completed install.
It validates existing install evidence and keeps the same unsafe-path, outstanding-claim and occupied-destination refusals.
A repeated request with no legacy state makes no changes.
Use it only with `install`; dry-run, force options, config-version updates and deprecated-file cleanup cannot be combined with it.
Then return to the newer dashboard's Hooks page, review the selected project and approve the separate policy and file-replacement choices.
...
On 2026-09-21, interactive Linux Codex CLI 0.155.1 delivered registered PreToolUse denials from both deny hooks after native project and handler review. Both probes were blocked before execution, harmless controls completed, and the model received the denial reasons. This capture supersedes the expired 0.149.0 evidence and expires at 2026-10-21T00:00:00Z; each row remains `scenario-unverified` until its fixed local scenario group passes. It renews neither Gruff nor Stop.

A trusted Codex CLI 0.149.1 `exec` capture subsequently completed `apply_patch`, ran Gruff, and delivered its analyzer-only marker through the changed PostToolUse registration; that evidence expires at 2026-09-25T20:17:22.830Z. Its registry gate is `scenario-unverified`; the fixed local scenario must also pass before the hook is effective.

On 2026-09-18, interactive Linux Codex CLI 0.154.0 delivered registered Stop feedback for a conflict, the model repaired the fixture during one automatic continuation, and clean Stop completed; that evidence expires on 2026-10-17. Its registry gate is `scenario-unverified`; the fixed local scenario must also pass before the hook is effective.

Exact configured-command replay does not upgrade provider-delivery evidence. Project-layer trust and handler trust remain separate, and app-server, remote execution, other platforms and modes, and forced-timeout behavior are unclaimed. See the [hook runtime matrix](../workflow/hooks/README.md#agent-event-name-mapping) for registered and disabled combinations.
...
Select the affected project on the Hooks page and use **Sync official hooks**; review requested policy changes and local-file replacements separately.
If Sync requires legacy-state migration, run the explicit `install --migrate-state-only` recovery action described above, then request a fresh review.
After Sync, change only the affected policy's switch and preserve the sibling choice. Finish any remaining install-file review before retrying install.

This sequence is fixture-verified with a complete pinned v1.16.0 Codex installation on a non-Windows host.
The fixture replays the exact saved general-policy handler against the upgraded project and the Git handler created by the upgrade.
It verifies independent off/on choices, retained user registrations, repeated Sync and target isolation; public-install fixtures cover storage recovery.
A saved choice or configured-command replay does not prove that an existing provider session reloads hooks.
Retry a benign request in that session and record its result; a fresh session is a separate control, not proof that the original session recovered.
Live provider recovery and native Windows replay remain unverified by these fixtures.
