...
`--dry-run` prints a read-only preview of the install write set as text or stable `goat-flow.managed-setup-preview.v2` JSON. Every row carries a repository-relative path, its manifest ownership, a state, and the proposed action and reason. Exact-copy `system-owned` templates are classified as `unchanged`, `local-edited`, `template-changed`, `both-changed`, `added`, `adopted`, `removed`, `missing`, or `unmanaged`. Destinations with no package template use three further states: `user-seeded` for a user-owned file install may create once, `user-preserved` for user-owned content install keeps, and `regenerated` for a generated file install rewrites from project state. A blocked preview exits 1; invalid flags exit 2. `--output` is the only optional dry-run write and writes the requested report, not setup state or installed files.
...
If the preview blocks, inspect the listed paths before choosing a write. `--force` admits inspected `system-owned` conflicts and nothing else: user-owned settings, config, hook choices, permission blocks, comments, and seeded guidance are preserved, and path-safety failures stay blocked. The preview itself needs no rollback because it changes nothing. Before a forced install, preserve the listed files with version control or a separate backup; after apply, use that same VCS/backup evidence to restore them if the result is not wanted.
