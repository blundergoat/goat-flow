...
- [x] Tag fragments as `create`/`fix`, filter in setup mode (Bug 1) — `FragmentKind` added, setup only emits `create`
- [x] Replace `"none"` fallback with conditional skip in fragments (Bug 2) — empty string fallback, fragments guard commands
- [x] Remove `markdown` from valid formats (Bug 3) — removed from parser, help, type, renderer

### Priority 2 — Scanner accuracy ~~(FIXED)~~
- [x] Fix line count off-by-one in `facts/agent.ts` (Bug 4) — trailing newline subtracted
- [x] Fix Ask First detection to search body content (Bug 5 — 3 pts) — searches `**Ask First**` in full content
- [x] Add `scripts/preflight-checks.sh` to shared facts (Bug 6 — 1 pt)
- [x] Add `CHANGELOG.md` to shared facts (Bug 7 — 1 pt)
- [x] Broaden Codex log-gate pattern (Bug 8 — 1 pt) — added `update.*log|log.*update|MUST.*log`

### Priority 3 — Design ~~(FIXED)~~
- [x] Remove `dependsOn` — was a no-op (Bug 9) — removed from types, fragments, and composer

### Priority 4 — Polish ~~(FIXED)~~
- [x] Add `[UNFILLED: name]` marker for unresolved template vars (Bug 13)
- [x] Add "N more" indicator to recommendation truncation (Bug 14) — already existed
- [x] Improve `test-all` pass/fail logic (Bug 15) — checks exit code AND error markers

### Priority 5 — Local Context Redesign → [M2.6](M2.6-local-context.md)

Old footgun-heuristic checks removed. Redesigned around `.github/instructions/` with baseline files (`code-review.instructions.md`, `git-commit.instructions.md`). See [M2.6-local-context.md](M2.6-local-context.md) for full plan.
...
- Bug 16: Version hardcoding — already fixed in prior commit

## Score After Fixes

| Agent | Before | After |
|-------|--------|-------|
| Claude | B (87%) | **A (100%)** — all tiers 100% |
| Codex | B (86%) | **A (98%)** — Foundation 100%, Full 100%, Standard 93% (missing stop-lint) |
| Gemini | A (91%) | **A (100%)** — all tiers 100% |
