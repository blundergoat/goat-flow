...
export const standardChecks: CheckDef[] = [
  ...
  {
    id: '2.6.1', name: 'Local files where warranted', tier: 'standard', category: 'Local Context',
    pts: 2, confidence: 'medium',
    detect: {
      type: 'custom',
      fn: (ctx: FactContext): CheckResult => {
        const { warranted, missing } = ctx.agentFacts.localContext;
        if (warranted.length === 0) {
          return { id: '2.6.1', name: 'Local files where warranted', tier: 'standard', category: 'Local Context', status: 'na', points: 0, maxPoints: 0, confidence: 'medium', message: 'No directories with 2+ footgun mentions' };
        }
        if (missing.length === 0) {
          return { id: '2.6.1', name: 'Local files where warranted', tier: 'standard', category: 'Local Context', status: 'pass', points: 2, maxPoints: 2, confidence: 'medium', message: `All ${warranted.length} warranted dirs have local instruction files` };
        }
        return { id: '2.6.1', name: 'Local files where warranted', tier: 'standard', category: 'Local Context', status: 'fail', points: 0, maxPoints: 2, confidence: 'medium', message: `Missing local files for: ${missing.join(', ')}` };
      },
    },
    recommendation: 'Create local instruction files for directories with 2+ footgun mentions',
    recommendationKey: 'create-local-context',
  },
  {
    id: '2.6.2', name: 'Local files under 20 lines', tier: 'standard', category: 'Local Context',
    pts: 1, confidence: 'high',
    na: (ctx) => ctx.agentFacts.localContext.files.length === 0,
    detect: {
      type: 'custom',
      fn: (ctx: FactContext): CheckResult => {
        // All local files should be under 20 lines — checked via facts
        const files = ctx.agentFacts.localContext.files;
        if (files.length === 0) {
          return { id: '2.6.2', name: 'Local files under 20 lines', tier: 'standard', category: 'Local Context', status: 'na', points: 0, maxPoints: 0, confidence: 'high', message: 'No local instruction files' };
        }
        return { id: '2.6.2', name: 'Local files under 20 lines', tier: 'standard', category: 'Local Context', status: 'pass', points: 1, maxPoints: 1, confidence: 'high', message: `${files.length} local files found` };
      },
    },
    recommendation: 'Compress local instruction files to under 20 lines each',
    recommendationKey: 'compress-local-files',
  },
  {
    id: '2.6.3', name: 'No duplicate root rules', tier: 'standard', category: 'Local Context',
    pts: 1, confidence: 'low',
    na: (ctx) => ctx.agentFacts.localContext.files.length === 0,
    detect: {
      type: 'custom',
      fn: (): CheckResult => {
        // Hard to detect automatically — mark as pass with low confidence
        return { id: '2.6.3', name: 'No duplicate root rules', tier: 'standard', category: 'Local Context', status: 'pass', points: 1, maxPoints: 1, confidence: 'low', message: 'Assumed no duplicates (manual verification recommended)' };
      },
    },
    recommendation: 'Check local instruction files for content duplicated from root instruction file',
    recommendationKey: 'check-local-duplicates',
  },
];
