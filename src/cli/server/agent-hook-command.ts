...
export function buildAgentHookCommand(
  ...
  // A provider without a config surface cannot receive the command the user enabled.
 ...
 * @param agent - selected provider; a null hook directory makes expected-command creation throw
 * @param spec - expected hook contract; empty metadata cannot produce an exact command
 * @returns true for byte-identical direct or nested commands; false means command repair is needed
 */
export function entryMatchesSpecCommand(
  ...
  if (commandEntryReferencesSpec(entry, spec)) {
    const expectedCommand = managedAgentHookCommand(agent, spec);
