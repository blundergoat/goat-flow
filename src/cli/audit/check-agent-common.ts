/**
 * Shared building blocks for the agent-setup check families.
 * Holds the provenance-record factories every agent check uses to stamp its evidence (so source type and verification date stay consistent across
 * checks), a path-dedupe helper, and the instruction-file precondition guard.
 *
 * Splitting these out keeps the per-check files focused on their own assertions rather than re-declaring boilerplate.
 */
...
/**
 * Build a `spec`-typed provenance record for an agent check - evidence whose authority is a written specification rather than a past incident.
 * Source type, empty URL list, the shared verification date, and `MUST` normative level are fixed; only the evidence paths vary per check.
 *
...
/**
 * Build an `incident`-typed provenance record for an agent check - evidence whose authority is a past failure captured in a footgun or lesson rather
 * than a spec.
 * Identical shape to `specProvenance` apart from the source type; the citation lives in `evidence_paths`, hence the empty URL list.
 *
 ...
 * Template-relative checks use this as a skip gate because an older bundle cannot authoritatively assess or repair skills and guardrails installed by
 * a newer release.
 *
