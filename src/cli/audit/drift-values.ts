 ...
 * Installed agent configuration is user-editable JSON and YAML, so anything read back from
 * it arrives as `unknown`. These guards are the single place that decides whether a value is
 * usable, keeping every drift check from re-inventing its own defensive checks.
 *
 * They are deliberately permissive about *what* is inside a value and strict only about its
 ...
 * Use on anything parsed out of a user's settings file, so a hand-edited array or string
 * becomes a reported difference rather than a runtime error.
 *
 * @param value - parsed config value of unknown shape; null and arrays are not objects here
 ...
 * Use before comparing a project's per-agent artifacts, so an unknown or misspelled agent
 * name is skipped rather than producing findings against something that cannot exist.
 *
