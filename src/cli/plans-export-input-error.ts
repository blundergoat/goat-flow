 ...
 *
 * Exporting a plan fails for two very different reasons: the person pointed at the wrong directory or wrote a milestone the parser cannot read, or
 * goat-flow has a bug.
 *
 * Only the first kind should reach them as a short message; the second should crash loudly with a stack trace so it gets reported rather than quietly
 * reworded into advice.
 *
 * This lives in its own module because both halves of the export flow raise it - the parser that reads milestones and the writer that checks
 * destinations - and neither should have to import the other just to throw.
 */
