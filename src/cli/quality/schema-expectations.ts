 ...
 * A saved report is JSON an agent produced, so every field arrives as `unknown` and could be
 * missing, the wrong type, or a value outside the accepted set. These helpers turn each
 * expectation into either a typed value or a precise error naming the field.
 *
 * The style is deliberately uniform: one helper per expectation, each reporting the field path
 * rather than throwing. A malformed report should tell its author exactly which key is wrong -
 * a stack trace would just tell them the tool broke, and they would have no way to fix it.
 */
